...
public:
	...
	static CDataFile FromVector(const char* szVariableName, const std::vector<float>& vData);
	...
	int GetData(const int&  iVariable, std::vector<float>& rVector);

	// Assigns rVector with the data at the target variable.
	// Returns the new size of rVector.  
	// Assumes readflag DF::RF_READ_AS_DOUBLE.
	// Returns -1 if an error is encountered.
	int GetData(const char* szVariableName, std::vector<float>& rVector);
	...
	// by iVariable.  Returns true if successful, false if an error is encountered.
	bool AppendData(const int& iVariable, const std::vector<float>& vData);
	...
	bool CreateVariable(const char* szName, const std::vector<float>& vData);
	...
	std::vector<float>& operator[](const int& in) { return m_v2dData.at(in); }
...
private:
	...
	int GetDataHeadNew(std::istream &is, DATA_FILE_DEV_INFO &tmpDataHead,int nVars);  // 对应新数据格式
...
private:
	...
	std::vector<std::vector<float> > m_v2dData;
public:

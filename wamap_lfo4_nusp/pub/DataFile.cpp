...
public:
	...
	AddTo(vector<vector<float> >& rTarget) : pvvTarget(&rTarget)    {}
	AddTo(vector<string>& rTarget)          : pvStrTarget(&rTarget)  {}
	AddTo(vector<float>& rTarget)          : pvTarget(&rTarget)     {}

	void operator()(const vector<string>& rSource)
		{ pvvStrTarget->push_back(rSource); }

	void operator()(const vector<float>& rSource)
...
private:
	...
	vector<vector<float> >* pvvTarget;
	vector<string>*          pvStrTarget;
	vector<float>*          pvTarget;
};
...
CDataFile CDataFile::FromVector(const char* szVariableName, const std::vector<float>& vData)
{
	CDataFile retDF;
	...
	retDF.m_v2dData.push_back(vector<float>());
...
CDataFile& CDataFile::operator=(const DF_SELECTION& dfs)
{
	...
	vector<vector<float> >().swap(m_v2dData);
	...
	for(iVar=firstVar; iVar<=lastVar; iVar++)
	{
		...
		if(dfs.pDataFile->m_dwReadFlags & RF_READ_AS_DOUBLE)
			m_v2dData.push_back(vector<float>());
...
// 			m_delim = "";
		...
		// 获取案例参数
		int nVars;
		m_DataHead.is_old_file = 0;
		m_DataHead.is_delim_end = 0;
		nVars = CountColsData(inFile, m_delim.at(0),m_DataHead.is_delim_end);
// 		cout<<"nVars1="<<nVars<<endl;
		nVars = CountColsData(inFile, m_delim.at(0),m_DataHead.is_delim_end);
// 		cout<<"nVars2="<<nVars<<endl;
		inFile.seekg(0);

		std::fstream::pos_type pos = GetDataHeadNew(inFile,m_DataHead,nVars);

// 		cout<<"[yjj]数据文件类型"<<m_DataHead.is_old_file<endl;
// 		printf("[yjj]数据文件类型[%d]\n",m_DataHead.is_old_file);
...
// 		cout<<"varOffset="<<varOffset<<endl;
		...
		m_vstrVariableNames.reserve(nVars);
		m_vstrSourceFilenames.reserve(nVars);
		m_v2dData.reserve(nVars);
		m_v2dStrData.reserve(nVars);
		for(iVar=0; iVar<nVars; iVar++)
		{
			...
			{
				m_v2dData.push_back(vector<float>());
				m_v2dData.back().reserve(nRowNum);
			}
			else if(m_dwReadFlags & RF_READ_AS_STRING)
			{
				m_v2dStrData.push_back(vector<string>());
				m_v2dStrData.back().reserve(nRowNum);
			}
			usleep(5);
		}
...
int CDataFile::GetDataHead( istream &is, DATA_FILE_DEV_INFO &tmpDataHead )
{
	...
	// 首行
	string tmpstring,strtimestart,strtimeend,strtimesample,tmpstring2;
	...
	for (int ii=0; ii<tmpDataHead.dev_num; ii++)
	{
		...
		tmpstring.erase(0,idx+1);
		idx = tmpstring.find_first_of(m_delim.at(0));
		tmpstring2 = tmpstring.substr(0,idx);
		tmpDataHead.vec_devid_s.push_back(ATOI(tmpstring2.c_str()));
	}
...
int CDataFile::GetDataHeadNew( std::istream &is, DATA_FILE_DEV_INFO &tmpDataHead ,int nVars)
{
...
// 	cout<<"nVars="<<nVars<<endl;
	for (iVar=0; iVar<nVars; iVar++)
	{
		is.getline(buff, sizeof buff, (iVar == nVars-1) ? '\n' : m_delim.at(0));
		tmpstring = buff;
		if (iVar == 0)
		{
			tmpDataHead.start_time = GetSecondByTimeString(tmpstring.c_str());
			continue;
		}
		else if (iVar == 1)
		{
			tmpDataHead.end_time = GetSecondByTimeString(tmpstring.c_str());
			continue;
		}
		else
		{
// 			cout<<"ID="<<tmpstring.c_str()<<endl;
			tmpDataHead.vec_devid_s.push_back(ATOI(tmpstring.c_str()));
		}
		usleep(5);
	}
...
// 	cout<<"dev_num="<<tmpDataHead.vec_devid_s.size()<<endl;
// 	for (iVar=0; iVar<tmpDataHead.vec_devid_s.size(); iVar++)
// 	{
// 		cout<<"id="<<tmpDataHead.vec_devid_s[iVar]<<endl;
// 	}
...
// 	}
	...
	for (iVar=0; iVar<nVars; iVar++)
	{
		...
		{
			tmpDataHead.vec_devvolt_s.push_back(tmpstring.c_str());
		}
		usleep(5);
	}
	pos = is.tellg();
...
// 	cout<<"tmpStartTime="<<tmpStartTime<<endl;
...
// 	cout<<"time_sample="<<tmpDataHead.time_sample<<endl;
...
// 			{
// 				// 末尾无分隔符,添加最后一个设备
// 				tmpDataHead.vec_devid_s.push_back(atol(tmpstring.c_str()));
// 			}
// 			break;
// 		}
// 		tmpDataHead.vec_devid_s.push_back(atol(tmpstring.substr(0,idx)));
// 		tmpstring.erase(0,idx +1);
// 	}
// 
// 	// 设备个数
...
// 		long tmpid;
// #ifdef _D5000
// 		tmpid = atol(tmpDataHead.vec_devid_s.front());
// #else
// 		tmpid = atoi(tmpDataHead.vec_devid_s.front());
// #endif
// 		if (tmpid < 1000000)
// 		{
// 			// 非设备ID，旧数据格式
...
istream& operator>>(istream& inFile, CDataFile& df)
{
	...
	{	
		...
		for(iVar=0; iVar<nVars; iVar++)
		{
			...
			{
				df.m_v2dData.push_back(vector<float>());
...
bool CDataFile::AppendData(const int& iVariable, const std::vector<float>& vData)
{
	try
	{
		for_each
...
bool CDataFile::CreateVariable(const char* szName, const double& initial_value, const int& iSize)
{
	try
	{
		...
		m_v2dData.push_back(vector<float>(iSize, initial_value));
		return true;
	}
...
bool CDataFile::CreateVariable(const char* szName, const std::vector<float>& vData)
{
	try
	{
		m_vstrSourceFilenames.push_back("");
...
int CDataFile::GetLargestVectorSize_() const
{
	...
	if(m_dwReadFlags & RF_READ_AS_DOUBLE)
	{
		std::vector<std::vector<float> >::const_iterator it = m_v2dData.begin();
...
void CDataFile::ClearData()
{
	...
	for (int ii = 0;ii< m_v2dStrData.size();ii++)
	{
		std::vector<std::string>().swap(m_v2dStrData.at(ii));
		m_v2dStrData.at(ii).clear();
	}
	for (int ii = 0;ii< m_v2dData.size();ii++)
	{
		vector<float>().swap(m_v2dData.at(ii));
		m_v2dData.at(ii).clear();
	}
	std::vector<std::vector<std::string> >().swap(m_v2dStrData);
	std::vector<std::vector<float> >().swap(m_v2dData);
	m_vstrVariableNames.clear();
	m_vstrSourceFilenames.clear();
	m_v2dStrData.clear();
	m_v2dData.clear();
...
// Returns -1 if an error is encountered.
int CDataFile::GetData(const int& iVariable, std::vector<float>& rVector)
{
	int retVal = 0;

	rVector.clear();
	rVector.reserve(m_v2dData.at(iVariable).size());

	try
	{
// 		rVector = m_v2dData.at(iVariable);
// 		rVector.assign(m_v2dData.at(iVariable).begin(),m_v2dData.at(iVariable).end());
		for (int ii=0;ii<m_v2dData.at(iVariable).size();ii++ )
		{
			rVector.push_back(m_v2dData.at(iVariable).at(ii));
		}
...
// Returns the new size of rVector if successful.
// Returns -1 if an error is encountered.
int CDataFile::GetData(const char* szVariableName, std::vector<float>& rVector)
{
	int index = LookupVariableIndex_(szVariableName);
...
// 		cout<<"nVars="<<nVars<<endl;
		inFile.seekg(0);
		std::fstream::pos_type pos = GetDataHeadNew(inFile,m_DataHead,nVars);
// 		cout<<"is_old_file="<<m_DataHead.is_old_file<<endl;
...
int CDataFile::ClearDataHead( DATA_FILE_DEV_INFO &tmpDataHead )
{
	...
	if (!tmpDataHead.vec_devid_s.empty())
	{
		vector<INT_REC>().swap(tmpDataHead.vec_devid_s);
		tmpDataHead.vec_devid_s.clear();
		vector<INT_REC>().swap(tmpDataHead.vec_devid_s);
	}
	if (!tmpDataHead.vec_devvolt_s.empty())
	{

...
#ifdef _LFO_HIS_LIB_
extern void PowAnaEnd(LFO_CASE_INFO& LfoCase);
extern void PowAnaDoing(LFO_CASE_INFO& LfoCase);
#endif

extern CLfoAnaObj* g_pAnaObj;

#ifndef _LFO_HIS_LIB_
extern 	CFileOpSop g_sop_fileop;
#endif
...
#ifdef _LFO_HIS_LIB_
extern	ThreadId_t  g_ThreadID[LFO_THREAD_NUM];
extern	int	 g_ThreadGetDataErr[LFO_THREAD_NUM];
extern	int   g_ThreadReturn;
extern	int	  g_ThreadFlag[LFO_THREAD_NUM];
extern	int   g_IsWarn;
#endif
...
int CLfoAnaBase::MonitorMainProc()
{
	...
	MessShow(m_LogString, LTINFO);
#ifdef _LFO_HIS_LIB_
	sprintf(m_LogString, "初始化结构体");
	MessShow(m_LogString, LTINFO);
	LFO_CASE_INFO temp_LfoCase;
	temp_LfoCase.event_id = 0;
	temp_LfoCase.stat = 0;
	memset(temp_LfoCase.case_desc, 0, sizeof(temp_LfoCase.case_desc));
	temp_LfoCase.lfo_gn_num = 0;
	temp_LfoCase.lfo_ln_num = 0;
	temp_LfoCase.lfo_tr_num = 0;
	if (!temp_LfoCase.vec_windows.empty())
	{
		ClearLfoSysInfo(temp_LfoCase.vec_windows);
	}
	temp_LfoCase.start_time = 0;
	temp_LfoCase.end_time = 0;
	temp_LfoCase.hold_time = 0;
	ClearLfoModeInfo(temp_LfoCase.lead_mode);
	ClearLfoDev(temp_LfoCase.warn_dev);
	if (!temp_LfoCase.vec_dev.empty())
	{
		ClearLfoDev(temp_LfoCase.vec_dev);
	}
#endif
	// 主循环
	while (true)
...
// 			g_pAnaObj->m_mutex.Unlock();
		...
		//终止：1)手动终止,2)三个子线程均出现读取数据失败(包括数据不刷新),3)振荡结束
		sprintf(m_LogString, "功率波动案例分析退出标志[%d],运行控制标志main_RunCtrCom[%d],getdata_err=[%d]", g_ThreadReturn, g_pAnaObj->main_RunCtrCom, getdata_err);
		MessShow(m_LogString, LTINFO);
		if (g_pAnaObj->main_RunCtrCom == MENU_LFO_PRO_CTL_STOP)
		{
			g_ThreadReturn = g_Thread_nun;
		}
		...
		if (getdata_err > 0)
		{
			...
			{
				ThreadId_t idid = GetId();
				sprintf(m_LogString, "第[%d]个线程[%ld][%ld]", ii + 1, idid, g_ThreadID[ii]);
				MessShow(m_LogString, LTINFO);

				if (GetId() == g_ThreadID[ii])
				{
					//本轮为第ii+1个线程
					//第ii+1个线程读取数据异常
					g_ThreadGetDataErr[ii] += 1;
					sprintf(m_LogString, "第[%d]个线程读取数据异常标志[%d]", ii + 1, g_ThreadGetDataErr[ii]);
					MessShow(m_LogString, LTINFO);
				}
			}
		}
		for (ii = 0; ii < g_Thread_nun; ii++)
		{
			sprintf(m_LogString, "第[%d]个线程读取数据异常标志[%d][%d]", ii + 1, g_ThreadFlag[ii], g_ThreadGetDataErr[ii]);
			MessShow(m_LogString, LTINFO);
			if (g_ThreadFlag[ii] == 0 && g_ThreadGetDataErr[ii] > 0)
			{
				g_ThreadReturn += 1;
				g_ThreadFlag[ii] = 1;
				sprintf(m_LogString, "第[%d]个线程标志[%d][%d]", ii + 1, g_ThreadFlag[ii], g_ThreadReturn);
				MessShow(m_LogString, LTINFO);
			}
		}
		sprintf(m_LogString, "功率波动案例分析2[%d]", g_ThreadReturn);
		MessShow(m_LogString, LTINFO);
		if (g_ThreadReturn >= g_Thread_nun)
		{
			if (g_IsWarn == 0 && m_ThreadIdx == 2)
			{
				sprintf(m_LogString, "GetDataErr_times=%d,功率波动案例分析结束,调用函数结束交互函数,参数为空", GetDataErr_times);
				MessShow(m_LogString, LTINFO);
				sprintf(m_LogString, "功率波动事件发生时间[%s],消失时间[%s],振荡设备个数[%d]", PT_GetTimeStringBySeconds(temp_LfoCase.start_time), PT_GetTimeStringBySeconds(temp_LfoCase.end_time), (int)temp_LfoCase.vec_dev.size());
				MessShow(m_LogString, LTINFO);
				PowAnaEnd(temp_LfoCase);	//该变量temp_LfoCase为空
// 				GetDataErr_times++;
// 				if (GetDataErr_times > 1)
// 				{
// 				}
// 				else
// 				{
// 					sprintf(m_LogString,"功率波动案例分析结束,即将运行最后一轮");
// 					MessShow(m_LogString,LTINFO);
// 					continue;
// 				}
			}
			sprintf(m_LogString, "g_ThreadReturn=%d,g_Thread_nun=%d,g_IsWarn=%d,m_ThreadIdx=%d,GetDataErr_times=%d", g_ThreadReturn, g_Thread_nun, g_IsWarn, m_ThreadIdx, GetDataErr_times);
			MessShow(m_LogString, LTINFO);
			break;
		}
...
// 			m_df.ClearData();
	...
	{
		sprintf(m_LogString, "支路[%s]id[%ld]", m_VecIntline[ii].line_name, m_VecIntline[ii].line_no);
		MessShow(m_LogString);
	...
	for (int ii = 0; ii < m_VecDev.size(); ii++)
	{
		...
		{
			// 			sprintf(m_LogString,"支路ID[%ld],断面ID[%ld]",m_VecIntline[jj].inf_no,m_VecDev[ii].dev.dev_id);
			// 			MessShow(m_LogString);
...
int CLfoAnaBase::GetGenCtlInfo(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (ii = 0; ii < (int)VecMonDev.size(); ii++)
	{
		...
		if (nFlag == 0)
		{
			sprintf(m_LogString, "未找到监视设备控制信息["INT_REC_FMT",%s]", INT_REC_VAL(VecMonDev[ii].dev.dev_id), VecMonDev[ii].dev.dev_name);
			MessShow(m_LogString, LTERROR);
		}
		// 		sprintf(m_LogString,"[yjj]机组设备["INT_REC_FMT",%s]状态[%d][%d][%d]",VecMonDev[ii].dev.dev_id,VecMonDev[ii].dev.dev_name,temf,VecMonDev[ii].stat,VecMonDev[ii].stat_old);
		// 		MessShow(m_LogString,LTERROR);
	}
...
int CLfoAnaBase::GetLineCtlInfo(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (ii = 0; ii < (int)VecMonDev.size(); ii++)
	{
		...
		// 		sprintf(m_LogString,"[yjj]线路设备["INT_REC_FMT",%s]状态[%d][%d][%d]",VecMonDev[ii].dev.dev_id,VecMonDev[ii].dev.dev_name,temf,VecMonDev[ii].stat,VecMonDev[ii].stat_old);
		// 		MessShow(m_LogString,LTERROR);
	}
...
int CLfoAnaBase::ShowPMUTime(LFO_SYS_INFO& MonSys)
{
	...
	sprintf(m_LogString, "PMU最新时间[%d][%s],本轮分析时间["INT_REC_FMT"~"INT_REC_FMT"][%s~%s]",
...
int CLfoAnaBase::GetTimeFromFile(LFO_SYS_INFO& MonSys, CDataFile& df)
{
	...
	sprintf(m_LogString, "案例总时间[%d],本轮已计算时间[%d],计算进度[%f]", df.m_DataHead.end_time - df.m_DataHead.start_time, MonSys.end_time - df.m_DataHead.start_time, MonSys.progress);
	MessShow(m_LogString);

	if (MonSys.start_time <= 0 || MonSys.end_time <= 0 || MonSys.end_time < MonSys.start_time)
	{
		sprintf(m_LogString, "分析时间错误["INT_REC_FMT"~"INT_REC_FMT"]", MonSys.start_time, MonSys.end_time);
		MessShow(m_LogString, LTERROR);
		return -1;
	}
...
// 		}
	...
	if (MonSys.start_time <= 0 || MonSys.end_time <= 0 || MonSys.end_time < MonSys.start_time)
	{
		sprintf(m_LogString, "分析时间错误[%d~%d]", MonSys.start_time, MonSys.end_time);
		MessShow(m_LogString, LTERROR);
		return -1;
	}
...
#endif
		// 		sprintf(m_LogString,"设备[%s][%ld]振幅限值[%f][%f]",VecMonDev[ii].dev.dev_name,VecMonDev[ii].dev.dev_id,VecMonDev[ii].dev.ampl_limit_s,VecMonDev[ii].dev.ampl_limit_e);
		// 		MessShow(m_LogString);
...
// 		}
		...
		if (mode_num == 0)
		{
			sprintf(m_LogString, "没有符合要求的振荡模式,case_id=["INT_REC_FMT"][%s],rank=[%d],snr=[%f],nDataNum=[%d]", VecCase[ii].dev.dev_id, VecCase[ii].dev.dev_name, rank, snr, nDataNum);
			MessShow(m_LogString, LTINFO);
		}
		else if (mode_num < 0)
		{
			sprintf(m_LogString, "无可行解,case_id=["INT_REC_FMT"]", VecCase[ii].dev.dev_id);
			MessShow(m_LogString, LTINFO);
		}
		else
		{
			...
			sprintf(m_LogString, "[%s]["INT_REC_FMT"]模式[%d]:", VecCase[ii].dev.dev_name, VecCase[ii].dev.dev_id, mode_num);
			MessShow(m_LogString, LTINFO);
			...
			{
				sprintf(m_LogString, "没有该频率段的模式["INT_REC_FMT"]", VecCase[ii].dev.dev_id);
				MessShow(m_LogString, LTERROR);
			}
			else
			{
...
// 					}
					...
					// 					sprintf(m_LogString,"%s["INT_REC_FMT"]tmpAmpl=%f,tmpWarnAmpl=%f,ampl_limit=%f,mode_save_amplper=%f",VecCase[ii].dev.dev_name,VecCase[ii].dev.dev_id,tmpAmpl,tmpWarnAmpl,VecMonDev[jj].dev.ampl_limit_s,m_LfoPara.mode_save_amplper);
					// 					MessShow(m_LogString,LTINFO);
...
// 						VecMonDev[jj].mode.end_time = m_MonSys.end_time;
					...
					{
						sprintf(m_LogString, "%s["INT_REC_FMT"]没有该频率段的模式[%f~%f],[%f,%f]", VecMonDev[jj].dev.dev_name, VecMonDev[jj].dev.dev_id,
					...
					// 					{
					// 						sprintf(m_LogString,"%s["INT_REC_FMT"]lead_mode_fdape[%f,%f,%f,%f,%f]",VecMonDev[jj].dev.dev_name,VecMonDev[jj].dev.dev_id,
...
int CLfoAnaBase::GetIntMode(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (int mm = 0; mm < m_VecIntline.size(); mm++)
	{
		...
		sprintf(m_LogString, "支路%s["INT_REC_FMT"]模式数[%d]", m_VecIntline[mm].line_name, m_VecIntline[mm].line_no, m_VecIntline[mm].vec_mode.size());
		MessShow(m_LogString, LTINFO);
	}
	for (int ii = 0; ii < VecMonDev.size(); ii++)
	{
		...
		sprintf(m_LogString, "%s["INT_REC_FMT"]主导模式频率[%.3f]初相位[%.2f]阻尼比[%.3f]", VecMonDev[ii].dev.dev_name, VecMonDev[ii].dev.dev_id, tmp_lead_mode.freq, tmp_lead_mode.phas, tmp_lead_mode.damp);
		MessShow(m_LogString, LTINFO);
		same_line_num = 0;
		...
		if (same_line_num == tmp_VecIntline.size())
		{
			...
			sprintf(m_LogString, "%s["INT_REC_FMT"]同调,频率[%f],阻尼比[%f],振幅[%f],均值[%f]", VecMonDev[ii].dev.dev_name, VecMonDev[ii].dev.dev_id, VecMonDev[ii].mode.freq, VecMonDev[ii].mode.damp, VecMonDev[ii].mode.ampl, VecMonDev[ii].curve.aver_val);
			MessShow(m_LogString, LTINFO);
		}
		else
		{
			//断面非同调
			sprintf(m_LogString, "%s["INT_REC_FMT"]非同调", VecMonDev[ii].dev.dev_name, VecMonDev[ii].dev.dev_id);
			MessShow(m_LogString, LTINFO);
			VecMonDev[ii].is_sm_md = MENU_LFO_MENU_NO;
			VecMonDev[ii].mode.freq = tmp_lead_mode.freq;
			VecMonDev[ii].mode.damp = VecMonDev[ii].mode.damp / same_line_num;
	...
	for (int mm = 0; mm < m_VecIntline.size(); mm++)
	{
		...
		// 		sprintf(m_LogString,"支路%s["INT_REC_FMT"]清空后模式数[%d]",m_VecIntline[mm].line_name,m_VecIntline[mm].line_no,m_VecIntline[mm].vec_mode.size());
		// 		MessShow(m_LogString,LTINFO);
	}
	return 0;
}
...
void CLfoAnaBase::EstimateSingleObjState(OSCILLAT_FLAG* osciFlag, LFO_LEAD_MODE* leadMode, int times, float SameFreqDifVal)
{
	...
	if (times == 1)
	{
		...
		sprintf(m_LogString, "obj_id="INT_REC_FMT", 阻尼比damping=%f, 频率frequency=%f[df=%f],主导模式振幅amplitude=%f,phase=%f,平均振幅aver_ampl=%f[%f]",
			leadMode->obj_id, leadMode->damping, leadMode->frequency, temp_freq_margin, leadMode->amplitude, leadMode->phase, leadMode->aver_ampl, LFO_Ampl_Per);
		MessShow(m_LogString);
		//振荡未出现或平息（幅值条件）
...
int CLfoAnaBase::JudgeSysLfoStat(vector<LFO_DEV_INFO> VecMonDev, vector<LFO_MODE_SHAPE> VecModeShape, LFO_SYS_INFO& MonSys, LFO_SYS_INFO& oscMonSys)
{
	...
	// 	sprintf(m_LogString,"本轮小扰动分析开始时间["INT_REC_FMT"],结束时间["INT_REC_FMT"]",oscMonSys.start_time,oscMonSys.end_time);
	// 	MessShow(m_LogString,LTINFO);
	// 		
	// 		// 增加设备小扰动统计时间
...
int CLfoAnaBase::WritePmuSqlData(LPO_DEV_INFO VecLPODEVINFO) //added 20141212
{
	...
	sprintf(sql_update, "insert into dsa_wams_powosc_dev(%s) values ("INT_REC_FMT","INT_REC_FMT","INT_REC_FMT","INT_REC_FMT","INT_REC_FMT",'%s','%s','%s','%s','%s',"INT_REC_FMT",%d,%d,%f,%f,%f,%f,%f,%f,%f,%d,TO_DATE('%s','YYYY-MM-DD HH24:MI:SS'),TO_DATE('%s','YYYY-MM-DD HH24:MI:SS'),'%s','%s')",
		LPODEVINFOINSERT, VecLPODEVINFO.dev_id, VecLPODEVINFO.fac_id_1, VecLPODEVINFO.area_id_1,
		VecLPODEVINFO.fac_id_2, VecLPODEVINFO.area_id_2, VecLPODEVINFO.dev_name, VecLPODEVINFO.fac_name_1, VecLPODEVINFO.fac_name_2,
		VecLPODEVINFO.area_name_1, VecLPODEVINFO.area_name_2, VecLPODEVINFO.dev_type, VecLPODEVINFO.is_monitor, VecLPODEVINFO.is_part_ana,
...
int CLfoAnaBase::WritePmuData(const char* mainstaresultfile, tab_lfo_pmu_data& mm_PMUData)
{
	...
	{
		fprintf(fp, ","INT_REC_FMT, mm_PMUData.head.vec_eleid[i]);
	}
	fprintf(fp, "\n");
...
int CLfoAnaBase::ReadLfoPara(vector<LFO_PARA_TABLE_READ>& VecLfoPara)
{
	...
	// 	sprintf(m_LogString,"【yjj】LFO参数表记录为空[%d][%d]",buf_base.GetLength(),sizeof(LFO_PARA_TABLE_READ));
	// 	MessShow(m_LogString,LTERROR);
...
int CLfoAnaBase::GetDevDataFromDB(INT_REC StartTime, INT_REC EndTime, int ordernum, LFO_DEV_INFO& MonDev)
{
	int iikk, nRealNum;
	short temp_nYcFilNo, temp_nYcColNo;
	int nNeedNum;
	nNeedNum = (EndTime - StartTime) * MAX_FRA_PER_SEC;
	if (nNeedNum <= 0)
	{
		sprintf(m_LogString, "时间段错误["INT_REC_FMT","INT_REC_FMT",%d]need[%d]", StartTime, EndTime, MAX_FRA_PER_SEC, nNeedNum);
		MessShow(m_LogString, LTERROR);
		return -1;
	}
	...
	{
		sprintf(m_LogString, "获取数据失败,dev["INT_REC_FMT",%s],YcNo[%d,%d],Time[%d,%d]", MonDev.dev.dev_id, MonDev.dev.dev_name,
	...
	{
		sprintf(m_LogString, "获取数据个数错误,dev["INT_REC_FMT",%s],YcNo[%d,%d],Time["INT_REC_FMT","INT_REC_FMT"],Need[%d],Real[%d]", MonDev.dev.dev_id, MonDev.dev.dev_name,
	...
	{
		sprintf(m_LogString, "数据状态错误,dev["INT_REC_FMT",%s],YcNo[%d,%d],Time["INT_REC_FMT","INT_REC_FMT"]", MonDev.dev.dev_id, MonDev.dev.dev_name,
			MonDev.dev.nYcFilNo[ordernum], MonDev.dev.nYcColNo[ordernum], StartTime, EndTime);
		MessShow(m_LogString, LTERROR);
		MonDev.stat = MENU_LFO_DEV_STS_BAD;
		SAFE_DELETE_ARRAY(pData);
...
{
	tmpIntline.recordid = 0;
	tmpIntline.inf_no = 0;
	tmpIntline.eletype = 0;
	tmpIntline.line_no = 0;
	tmpIntline.onoff = 0;
	tmpIntline.mpoint = 0;
	tmpIntline.stat_no = 0;
...
// 			MessShow(m_LogString,LTINFO);
			...
			if (idx != string::npos)
			{
				tmpDevInfo.dev.dev_id = a_to_d(tmpstring.substr(0, idx).c_str());
...
// 			tmpstring = df.m_DataHead.vec_devid_s[ii];
			...
			tmpDevInfo.dev_info.dev_id = tmpDevInfo.dev.dev_id;
			// 			sprintf(m_LogString,"设备ID=[%ld]",tmpDevInfo.dev.dev_id);
			// 			MessShow(m_LogString,LTINFO);
			...
			sprintf(m_LogString, "[yjj]GetDevFromFile设备ID["INT_REC_FMT"],索引["INT_REC_FMT"]", tmpDevInfo.dev.dev_id, tmpDevInfo.dev.dev_idx);
			MessShow(m_LogString, LTINFO);
			// 获取测点名称
			// 去掉前两列"日期时间,毫秒"
			tmpstring.clear();
		...
		if (nFlag == 0)
		{
			...
			// 			sprintf(m_LogString,"设备[%s]索引[%ld]",tmpDevInfo.dev.dev_name,tmpDevInfo.dev.dev_idx);
			// 			MessShow(m_LogString);
	...
	vector<long> devid;
	devid.clear();
	tmpstring = m_LfoPara.devicefile_name;
	...
	for (int ii = 0; ii < m_VecIntline.size(); ii++)
	{
		...
		if (!bfind)
		{
			...
			tmpdev.dev.dev_type = MENU_LFO_DEV_ITFC;
			tmpvecdev.push_back(tmpdev);
			sprintf(m_LogString, "添加断面:%s[%ld]", tmpdev.dev.dev_name, tmpdev.dev.dev_id);
			MessShow(m_LogString);
		}
		sprintf(m_LogString, "添加断面支路:%s[%ld]", m_VecIntline[ii].line_name, m_VecIntline[ii].line_no);
		MessShow(m_LogString);
	}
...
int CLfoAnaObj::GetDevice(const string filename, vector<long>& VecDevID, vector<LFO_INTLINE_INFO>& VecINTLINE)
{
	...
	{
		sprintf(m_LogString, "监视设备文件[%s]不存在", tmpstring.c_str());
		MessShow(m_LogString, LTERROR);
		return -1;
	}
	...
	sprintf(m_LogString, "文件包含[%d]行", vtxt.size());
	MessShow(m_LogString);
	for (int ii = 0; ii < vtxt.size(); ii++)
	{
		if (vtxt.at(ii).find("[监视设备信息]") != std::string::npos)
		{
			vdevice.assign(vtxt.begin() + ii, vtxt.end());
		}
		if (vtxt.at(ii).find("[断面支路信息]") != std::string::npos)
		{
			vintline.assign(vtxt.begin() + ii, vtxt.end());
			break;
		}
	}
	//读取监视设备信息
	...
	for (int ii = 0; ii < vdevice.size(); ii++)
	{
		...
		vDataVal.clear();
		PS_StringTrimAll(vdevice.at(ii));
		while (vdevice.at(ii).find(";") == (int)vdevice.at(ii).size() - 1)
		{//去掉最后的";"
			if (vdevice.at(ii).size() != 0)
			{
				vdevice.at(ii).erase(vdevice.at(ii).size() - 1, 1);
			}
		}
		PS_StringDivde(vdevice.at(ii), ",", vDataVal);
		if (vDataVal.size() < 2) continue;
		VecDevID.push_back(atol(vDataVal.at(0).c_str()));
		sprintf(m_LogString, "监视设备:%s[%s]", vDataVal.at(1).c_str(), vDataVal.at(0).c_str());
		MessShow(m_LogString);
	}
	sprintf(m_LogString, "监视设备数量%d", VecDevID.size());
	MessShow(m_LogString);
	//读取断面支路信息
	for (int ii = 0; ii < vintline.size(); ii++)
	{
		...
		vDataVal.clear();
		while (vintline.at(ii).find(";") == (int)vintline.at(ii).size() - 1)
		{//去掉最后的";"
			if (vintline.at(ii).size() != 0)
			{
				vintline.at(ii).erase(vintline.at(ii).size() - 1, 1);
			}
		}
		int ret = PS_StringDivde(vintline.at(ii), ",", vDataVal);
		if (vDataVal.size() < 8) continue;
		tmpintline.inf_no = atol(vDataVal.at(0).c_str());
		sprintf(tmpintline.inf_name, "%s", vDataVal.at(1).c_str());
		tmpintline.eletype = atoi(vDataVal.at(2).c_str());
		tmpintline.line_no = atol(vDataVal.at(3).c_str());
		sprintf(tmpintline.line_name, "%s", vDataVal.at(4).c_str());
		tmpintline.mpoint = atoi(vDataVal.at(5).c_str());
		tmpintline.stat_no = atol(vDataVal.at(6).c_str());
		tmpintline.onoff = atoi(vDataVal.at(7).c_str());
		VecINTLINE.push_back(tmpintline);
	}
	sprintf(m_LogString, "断面支路数量%d", VecINTLINE.size());
	MessShow(m_LogString);
	return 0;
}
	...
	for (ii = 0; ii < (int)VecDev.size(); ii++)
	{
		if (VecDev[ii].dev_info.dev_type == MENU_LFO_DEV_GEN)
		{
			if (m_LfoPara.is_add_facname == 1)
			{
				sprintf(m_LogString, "%d\t\t%s%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%d\t\t%d",
					ii + 1, VecDev[ii].dev_info.fac_name, VecDev[ii].dev_info.dev_name, VecDev[ii].dev_info.dev_id, VecDev[ii].dev_info.fac_name, VecDev[ii].dev_info.fac_id,
					VecDev[ii].dev_info.vflag, VecDev[ii].dev_info.yc_flag);
			}
			else
			{
				sprintf(m_LogString, "%d\t\t%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%d\t\t%d",
					ii + 1, VecDev[ii].dev_info.dev_name, VecDev[ii].dev_info.dev_id, VecDev[ii].dev_info.fac_name, VecDev[ii].dev_info.fac_id,
					VecDev[ii].dev_info.vflag, VecDev[ii].dev_info.yc_flag);
			}
			//fs<<m_LogString<<endl;
			sprintf(m_LogString, "%d\t\t%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%d\t\t%d",
		...
		else if (VecDev[ii].dev_info.dev_type == MENU_LFO_DEV_LINE)
		{
			sprintf(m_LogString, "%d\t\t%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%d\t\t%d\t\t%d\t\t%d",
				ii + 1, VecDev[ii].dev_info.dev_name, VecDev[ii].dev_info.dev_id, VecDev[ii].dev_info.dot1.fac_name, VecDev[ii].dev_info.dot1.dot_id, VecDev[ii].dev_info.dot2.fac_name, VecDev[ii].dev_info.dot2.dot_id,
		...
		else if (VecDev[ii].dev_info.dev_type == MENU_LFO_DEV_TR)
		{
			if (m_LfoPara.is_add_facname == 1)
			{
				sprintf(m_LogString, "%d\t\t%s%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%d\t\t%d\t\t%d\t\t%d",
					ii + 1, VecDev[ii].dev_info.fac_name, VecDev[ii].dev_info.dev_name, VecDev[ii].dev_info.dev_id, VecDev[ii].dev_info.dot1.dot_name, VecDev[ii].dev_info.dot1.dot_id, VecDev[ii].dev_info.dot2.dot_name, VecDev[ii].dev_info.dot2.dot_id,
					VecDev[ii].dev_info.dot3.dot_name, VecDev[ii].dev_info.dot3.dot_id, VecDev[ii].dev_info.vflag, VecDev[ii].dev_info.yc_flag, VecDev[ii].dev_info.is_old, VecDev[ii].dev_info.is_add);
			}
			else
			{
				sprintf(m_LogString, "%d\t\t%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%s["INT_REC_FMT"]\t\t%d\t\t%d\t\t%d\t\t%d",
					ii + 1, VecDev[ii].dev_info.dev_name, VecDev[ii].dev_info.dev_id, VecDev[ii].dev_info.dot1.dot_name, VecDev[ii].dev_info.dot1.dot_id, VecDev[ii].dev_info.dot2.dot_name, VecDev[ii].dev_info.dot2.dot_id,
					VecDev[ii].dev_info.dot3.dot_name, VecDev[ii].dev_info.dot3.dot_id, VecDev[ii].dev_info.vflag, VecDev[ii].dev_info.yc_flag, VecDev[ii].dev_info.is_old, VecDev[ii].dev_info.is_add);
			}
			//MessShow(m_LogString,LTDEV);
			fs << m_LogString << endl;
...
int CLfoAnaObj::ThreadInit()
{
	...
	for (ii = 0; ii < g_Thread_nun; ii++)
	{
		...
		m_pLfoThread[ii].m_LfoPara.prony_para.freq_min = m_pLfoThread[ii].m_LfoPara.freq_min;
		m_pLfoThread[ii].m_LfoPara.prony_para.freq_max = m_pLfoThread[ii].m_LfoPara.freq_max;
...
// 	m_pLfoThreadInfo[threadidx].MonSys = MonSys;
	...
	else if (m_MonSys.stat_old >= MENU_LFO_DEV_STS_WRN
		&& m_MonSys.stat >= MENU_LFO_DEV_STS_WRN)
	{
		...
		&& m_MonSys.stat <= MENU_LFO_DEV_STS_SCT)
	{
	...
	else if (m_MonSys.stat_old <= MENU_LFO_DEV_STS_SCT
		&& m_MonSys.stat <= MENU_LFO_DEV_STS_SCT)
	{
		// 安全->安全,空闲
		m_MonSys.save_flag = LFO_SAVE_FLAG_IDEL;
...
// 		MessShow(m_LogString,LTINFO);
		...
		else
		{
			...
			if (m_pLfoThreadInfo[ii].MonSys.stat >= MonSys.stat)
			{
				...
				MonSys.progress = m_pLfoThreadInfo[ii].MonSys.progress;
...
// 		MonSys.lead_dev = MonSys.vec_dev.front();
		...
		if (m_LfoPara.is_add_facname == 1)
		{
			...
			{
				if (MonSys.lead_gen.dev.dev_id != 0)
				{//有机组但机组不是最严重设备
					// 增加厂站名称
					if (m_LfoPara.is_eng == 1)
					{
	...
	sprintf(m_LogString, "windows_id["INT_REC_FMT"]freq[%.3f,%.3f]time["INT_REC_FMT"~"INT_REC_FMT"][%s~%s]fdap[%.3f,%.3f,%.3f,%.3f]stat[%d]desc[%s]",
...
int CLfoAnaObj::FormLfoCaseResult(vector<LFO_SYS_INFO>& VecLfoWindow, LFO_CASE_INFO& LfoCase, const int save_flag)
{
	...
	else if (save_flag == LFO_SAVE_FLAG_START)
	{
		...
		LfoCase.progress = tmpWindows.progress;
...
// 						}
						...
						//						}
						sprintf(m_LogString, "[提示]新增设备0["INT_REC_FMT"],振幅[%f],耗散功率[%f][%f],是否振荡源[%d]", LfoCase.vec_dev[jj].dev.dev_id, LfoCase.vec_dev[jj].mode.ampl, LfoCase.vec_dev[jj].mode.pow, tmpWindows.lead_dev.mode.pow, LfoCase.vec_dev[jj].mode.is_source);
						MessShow(m_LogString, LTINFO);
						break;
					}
				}
				if (nFlag == 0)
				{
					sprintf(m_LogString, "[提示]新增设备1["INT_REC_FMT"],振幅[%f],耗散功率[%f],是否振荡源[%d]", tmpWindows.vec_dev[ii].dev.dev_id, tmpWindows.vec_dev[ii].mode.ampl, tmpWindows.vec_dev[ii].mode.pow, tmpWindows.vec_dev[ii].mode.is_source);
					MessShow(m_LogString, LTINFO);
					// 新增设备
...
// 						LfoCase.cen_dev[jj].mode = tmpWindows.cen_dev[ii].mode;
						...
						//						}
						sprintf(m_LogString, "[提示]新增厂站0["INT_REC_FMT"],振幅[%f],是否振荡中心[%d]", LfoCase.cen_dev[jj].dev.dev_id, LfoCase.cen_dev[jj].mode.ampl, LfoCase.cen_dev[jj].lfo_is_cntr);
						MessShow(m_LogString, LTINFO);
						break;
					}
				}
				if (nFlag == 0)
				{
					sprintf(m_LogString, "[提示]新增厂站1["INT_REC_FMT"],振幅[%f],是否振荡中心[%d]", tmpWindows.cen_dev[ii].dev.dev_id, tmpWindows.cen_dev[ii].mode.ampl, tmpWindows.cen_dev[ii].lfo_is_cntr);
					MessShow(m_LogString, LTINFO);
					// 新增设备
					//memset((void *)&tmpDev,0,sizeof(LFO_DEV_INFO));
					ClearLfoDev(tmpDev);
...
#endif
				sprintf(m_LogString, "本次振荡事件案例描述:%s", LfoCase.case_desc);
				MessShow(m_LogString, LTINFO);
			}
		}
...
// 		}
	...
	else if (save_flag == LFO_SAVE_FLAG_HOLD)
	{
		...
		LfoCase.stat = tmpWindows.stat;
		//更新计算进度
		if (LfoCase.progress < tmpWindows.progress)
		{
			LfoCase.progress = tmpWindows.progress;
		}
...
// 		}
		...
		for (ii = 0; ii < (int)LfoCase.vec_windows.size(); ii++)
		{
			//			sprintf(m_LogString,"[yjj2]持续振荡窗口ID["INT_REC_FMT"],已有窗口["INT_REC_FMT"]",tmpWindows.window_id,LfoCase.vec_windows[ii].window_id);
			//			MessShow(m_LogString,LTINFO);	
			if (tmpWindows.window_id == LfoCase.vec_windows[ii].window_id)
			{
				sprintf(m_LogString, "持续振荡窗口ID["INT_REC_FMT"]重复,该窗口振荡设备个数%d,已有窗口振荡设备个数%d", tmpWindows.window_id, tmpWindows.vec_dev.size(), LfoCase.vec_windows[ii].vec_dev.size());
				MessShow(m_LogString, LTINFO);
				nFlag = 1;
				break;
			}
		...
		if (nFlag == 0)
		{
			...
			for (ii = 0; ii < (int)tmpWindows.vec_dev.size(); ii++)
			{
				...
				for (jj = 0; jj < (int)LfoCase.vec_dev.size(); jj++)
				{
					if (tmpWindows.vec_dev[ii].dev.dev_id == LfoCase.vec_dev[jj].dev.dev_id)
					{
						...
						// 更新持续时间
						LfoCase.vec_dev[jj].hold_time = LfoCase.vec_dev[jj].end_time - LfoCase.vec_dev[jj].start_time;
						// 						sprintf(m_LogString,"[yjj]11["INT_REC_FMT"]，开始时间["INT_REC_FMT"],结束时间["INT_REC_FMT"],持续时间[%d]", LfoCase.vec_dev[jj].dev.dev_id,LfoCase.vec_dev[jj].start_time,LfoCase.vec_dev[jj].end_time,LfoCase.vec_dev[jj].hold_time);
						// 						MessShow(m_LogString,LTINFO);						
						...
						LfoCase.vec_dev[jj].vec_lead_mode.push_back(tmpWindows.vec_dev[ii].mode);
						//						}
						//  		sprintf(m_LogString,"[yjj]新增设备2["INT_REC_FMT"]，振幅[%f],耗散功率[%f][%f]",LfoCase.vec_dev[jj].dev.dev_id,tmpWindows.lead_dev.mode.ampl,LfoCase.vec_dev[jj].mode.pow,tmpWindows.lead_dev.mode.pow);
						//  		MessShow(m_LogString,LTINFO);
						break;
					}
				}
				if (nFlag == 0)
				{
					sprintf(m_LogString, "[提示]新增设备3["INT_REC_FMT"],振幅[%f],耗散功率[%f]", tmpWindows.vec_dev[ii].dev.dev_id, tmpWindows.vec_dev[ii].mode.ampl, tmpWindows.vec_dev[ii].mode.pow);
					MessShow(m_LogString, LTINFO);
					// 新增设备
					//memset((void *)&tmpDev,0,sizeof(LFO_DEV_INFO));
					ClearLfoDev(tmpDev);
			...
			sprintf(m_LogString, "汇总后振荡设备个数%d", LfoCase.vec_dev.size());
			MessShow(m_LogString, LTINFO);
...
// 						LfoCase.cen_dev[jj].mode = tmpWindows.cen_dev[ii].mode;
		...
		for (ii = 0; ii < (int)LfoCase.vec_dev.size(); ii++)
		{
			sprintf(m_LogString, "[提示]2[%s]是否振荡源[%d],设备类型[%d]", LfoCase.vec_dev[ii].dev.dev_name, LfoCase.vec_dev[ii].mode.is_source, LfoCase.vec_dev[ii].dev.dev_type);
			MessShow(m_LogString, LTINFO);
			if (LfoCase.vec_dev[ii].dev.dev_type == MENU_LFO_DEV_GEN)
			{
				LfoCase.lfo_gn_num++;
...
// 		sprintf(m_LogString,"[提示]事件ID["INT_REC_FMT"]",LfoCase.event_id);
// 		MessShow(m_LogString,LTINFO);
...
// 		}
	...
	sprintf(m_LogString, "振荡中心描述[%s]振荡源描述[%s]", tmpLfoParaInfo.center_desc2, tmpLfoParaInfo.lfos_des2);
	MessShow(m_LogString);
...
int CLfoAnaObj::UpdateLfoFcTable(LFO_CASE_INFO& LfoCase, int save_flag)
{
	...
	for (ii = 0; ii < (int)tmpVecLfoFacInfo.size(); ii++)
	{
		...
		for (jj = 0; jj < (int)LfoCase.cen_dev.size(); jj++)
		{
			sprintf(m_LogString, "UpdateLfoFcTable设备[%s]类型[%d]id[%ld]", LfoCase.cen_dev[jj].dev.dev_name, LfoCase.cen_dev[jj].dev.dev_type, LfoCase.cen_dev[jj].dev.dev_id);
			MessShow(m_LogString);
			...
			if (tmpVecLfoFacInfo[ii].fac_id == LfoCase.cen_dev[jj].dev.dev_id || tmpVecLfoFacInfo[ii].set_bus_id == LfoCase.cen_dev[jj].dev.dev_id)
			{
				...
				nFlag = 1;
				sprintf(m_LogString, "2写LFO厂站表第[%d]个设备索引["INT_REC_FMT"],是否振荡中心[%d]", ii, tmpVecLfoFacInfo[ii].fac_idx, tmpLfoDev.lfo_is_cntr);
				MessShow(m_LogString);
				break;
			}
		}
...
int CLfoAnaObj::UpdateLfoGnTable(LFO_CASE_INFO& LfoCase, int save_flag)
{
	...
	for (ii = 0; ii < (int)tmpVecLfoGenInfo.size(); ii++)
	{
		...
		for (jj = 0; jj < (int)LfoCase.vec_dev.size(); jj++)
		{
			...
			//MessShow(m_LogString);
	//   	sprintf(m_LogString, "[提示]LFO发电机表设备ID["INT_REC_FMT"]=["INT_REC_FMT"]",tmpVecLfoGenInfo[ii].gn_id,LfoCase.vec_dev[jj].dev.dev_id);
	//		MessShow(m_LogString);
			...
			if (tmpVecLfoGenInfo[ii].gn_id == LfoCase.vec_dev[jj].dev.dev_id)
			{
				...
				sprintf(m_LogString, "1写LFO发电机表第[%d]个设备索引["INT_REC_FMT"],名称[%s],最大摆动[%f],最大值[%f],最小值[%f],是否振荡源(事件)[%d],是否振荡源(窗口)[%d]", ii, tmpVecLfoGenInfo[ii].gn_idx, LfoCase.vec_dev[jj].dev.dev_name, tmpLfoDev.lfo_max_min_dif, tmpLfoDev.lfo_max_val, tmpLfoDev.lfo_min_val, tmpLfoDev.is_lfo_source2, LfoCase.vec_dev[jj].mode.is_source);
				MessShow(m_LogString);
				nFlag = 1;
				break;
			}
		...
		//  		sprintf(m_LogString, "2写LFO发电机表第[%d]个设备索引["INT_REC_FMT"],是否振荡源[%d],振幅[%f],能量[%f]",ii,tmpVecLfoGenInfo[ii].gn_idx,tmpLfoDev.is_lfo_source,tmpLfoDev.lead_ampl,tmpLfoDev.lead_gen_pow);
		//  		MessShow(m_LogString);
	...
	for (ii = 0; ii < (int)tmpVecLfoLineInfo.size(); ii++)
	{
		...
		for (jj = 0; jj < (int)LfoCase.vec_dev.size(); jj++)
		{
			...
			if (tmpVecLfoLineInfo[ii].acln_id == LfoCase.vec_dev[jj].dev.dev_id)
			{
				...
				// 				sprintf(m_LogString, "写LFO交流线路表设备["INT_REC_FMT"]持续时间[%s]",tmpVecLfoLineInfo[ii].acln_id,tmpLfoDev.lfo_time);
				// 		 		MessShow(m_LogString);	
				nFlag = 1;
				break;
			}
		...
		//   		sprintf(m_LogString, "2写LFO交流线路表第[%d]个设备索引["INT_REC_FMT"]",ii,tmpVecLfoLineInfo[ii].acln_idx);
		//  		MessShow(m_LogString);		
...
int CLfoAnaObj::SaveCaseCurve(LFO_CASE_INFO& LfoCase)
{
	...
	for (ii = 0; ii < (int)tmpVecYc.size(); ii++)
	{
		...
		else if (table_no == ACLN_DOT_NO_TAB)
		{
			...
			for (jj = 0; jj < (int)LfoCase.vec_dev.size(); jj++)
			{
				if (tmpdev.dev_id == LfoCase.vec_dev[jj].dev_info.dot1.dot_id || tmpdev.dev_id == LfoCase.vec_dev[jj].dev_info.dot2.dot_id)
				{
					tmpdev.dev_id1 = LfoCase.vec_dev[jj].dev_info.dev_id;
					tmpdev.is_warn = 1;
					break;
				}
			}
		...
		else if (table_no == TR_DEVICE_NO_TAB || table_no == TRWD_DEVICE_NO_TAB)
		{
			...
			{
				if (tmpdev.dev_id == LfoCase.vec_dev[jj].dev_info.dot1.dot_id || tmpdev.dev_id == LfoCase.vec_dev[jj].dev_info.dot2.dot_id
					|| tmpdev.dev_id == LfoCase.vec_dev[jj].dev_info.dot3.dot_id)
				{
					tmpdev.dev_id1 = LfoCase.vec_dev[jj].dev_info.dev_id;
					tmpdev.is_warn = 1;
					break;
	...
	fs << "触发告警设备ID = " << tmpCase.warn_dev_id << ";" << endl;
	...
	fs << "lowfreq_min = " << m_LfoPara.lfo_freq_min << ";" << endl; /* 低频振荡频率下限 */
	fs << "lowfreq_max = " << m_LfoPara.lfo_freq_max << ";" << endl; /* 低频振荡频率上限  */
	...
	for (ii = 0; ii < (int)tmpCase.vecdev.size(); ii++)
	{
		...
		fs << tmpCase.vecdev[ii].dev_id << " ";			//设备端点ID
	...
	for (i = 0; i < (int)VecLfoFcTab.size(); i++)
	{
		...
		for (jj = 0; jj < (int)VecFacInfo.size(); jj++)
		{
			if (tmpDevInfo.dev_id == VecFacInfo[jj].fac_id)
			{
				...
				nFlag = 1;
				// 				sprintf(m_LogString,"[yjj]获取机组名称["INT_REC_FMT"]",tmpDevInfo.dev_id);
				// 				MessShow(m_LogString,LTERROR);
				break;
			}
		...
		if (nFlag == 0)
		{
			sprintf(m_LogString, "未找到对厂站名称["INT_REC_FMT"]", tmpDevInfo.dev_id);
			MessShow(m_LogString, LTERROR);
		}
		...
		if (nFlag2 == 0)
		{
			...
			MessShow(m_LogString, LTERROR);
			tmpDevInfo.dot1.dot_id = 0;
			tmpDevInfo.dot1.fac_id = 0;
			// 			tmpDevInfo.dot1.area_id = 0;
		}
...
int CLfoAnaObj::AddLfoGen(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (i = 0; i < (int)VecLfoGen.size(); i++)
	{
		...
		for (jj = 0; jj < (int)VecGenInfo.size(); jj++)
		{
			if (tmpDevInfo.dev_id == VecGenInfo[jj].gn_id)
			{
				...
				for (kk = 0; kk < (int)VecVLTypeTab.size(); kk++)
				{
					...
					{
						// 						sprintf(m_LogString,"[yjj]获取机组名称["INT_REC_FMT"][%f]",tmpDevInfo.dev_id,VecVLTypeTab[kk].vbase);
						// 						MessShow(m_LogString,LTERROR);
		...
		if (nFlag == 0)
		{
			sprintf(m_LogString, "未找到对应发电机名称["INT_REC_FMT"]", tmpDevInfo.dev_id);
			MessShow(m_LogString, LTERROR);
		}
...
int CLfoAnaObj::SetFacInfo(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (ii = 0; ii < (int)VecMonDev.size(); ii++)
	{
		if (VecMonDev[ii].dev_info.dev_type == MENU_LFO_DEV_GEN)
		{
			...
			if (nFlag1 == 0)
			{
				sprintf(m_LogString, "未找到对应厂站名称["INT_REC_FMT"]", VecMonDev[ii].dev_info.fac_id);
				MessShow(m_LogString, LTERROR);
		...
		else if (VecMonDev[ii].dev_info.dev_type == MENU_LFO_DEV_LINE)
		{
			...
			if (nFlag1 == 0)
			{
				sprintf(m_LogString, "未找到对应厂站名称["INT_REC_FMT"]", VecMonDev[ii].dev_info.dot1.fac_id);
				MessShow(m_LogString, LTERROR);
				sprintf(VecMonDev[ii].dev_info.dot1.fac_name, "%s", "未匹配");
			}
			if (nFlag2 == 0)
			{
				sprintf(m_LogString, "未找到对应厂站名称["INT_REC_FMT"]", VecMonDev[ii].dev_info.dot2.fac_id);
				MessShow(m_LogString, LTERROR);
...
int CLfoAnaObj::SetAreaInfo(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (ii = 0; ii < (int)VecMonDev.size(); ii++)
	{
		if (VecMonDev[ii].dev_info.dev_type == MENU_LFO_DEV_GEN)
		{
			...
			if (nFlag1 == 0)
			{
				sprintf(m_LogString, "未找到对应区域名称["INT_REC_FMT"]", VecMonDev[ii].dev_info.area_id);
				MessShow(m_LogString, LTERROR);
...
int CLfoAnaObj::SetFesYcInfo(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (ii = 0; ii < (int)VecMonDev.size(); ii++)
	{
		if (VecMonDev[ii].dev_info.dev_type == MENU_LFO_DEV_GEN)
		{
			...
			if (VecMonDev[ii].dev_info.dot1.dot_id <= 0)
			{
				sprintf(m_LogString, ""INT_REC_FMT",%s信息不全", VecMonDev[ii].dev_info.dev_id, VecMonDev[ii].dev_info.dev_name);
				MessShow(m_LogString, LTINFO);
				continue;
			}
			...
			{
				sprintf(m_LogString, "未找到对应测点信息或测点信息不全["INT_REC_FMT"][%s]", VecMonDev[ii].dev_info.dot1.dot_id, VecMonDev[ii].dev_info.dot1.dot_name);
				MessShow(m_LogString, LTERROR);
			}
			else
			{
		...
		else if (VecMonDev[ii].dev_info.dev_type == MENU_LFO_DEV_LINE)
		{
			...
			if (VecMonDev[ii].dev_info.dot1.dot_id <= 0 && VecMonDev[ii].dev_info.dot2.dot_id <= 0)
			{
				sprintf(m_LogString, ""INT_REC_FMT",%s信息不全", VecMonDev[ii].dev_info.dev_id, VecMonDev[ii].dev_info.dev_name);
				MessShow(m_LogString, LTINFO);
				continue;
...
int CLfoAnaObj::SetDevInfo(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (ii = 0; ii < (int)VecMonDev.size(); ii++)
	{
		...
		else if (VecMonDev[ii].dev.dev_type == MENU_LFO_DEV_FAC)
		{
			...
			{
				VecMonDev[ii].stat = MENU_LFO_DEV_STS_NOYC;
				sprintf(m_LogString, "厂站静态信息:名称[%s],ID[%ld],无测点", VecMonDev[ii].dev.dev_name, VecMonDev[ii].dev.dev_id);
				MessShow(m_LogString, LTERROR);
				continue;
			}
			else
...
int CLfoAnaObj::AddLfoLine(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (i = 0; i < (int)VecLfoLine.size(); i++)
	{
		...
		if (nFlag == 0)
		{
			sprintf(m_LogString, "未找到对应交流线路名称["INT_REC_FMT"]", tmpDevInfo.dev_id);
			MessShow(m_LogString, LTERROR);
		}
		...
		for (jj = 0; jj < (int)VecVLTypeTab.size(); jj++)
		{
			...
			{
				// 				sprintf(m_LogString,"[yjj]线路名称["INT_REC_FMT"][%f]",tmpDevInfo.dev_id,VecVLTypeTab[jj].vbase);
				// 				MessShow(m_LogString,LTERROR);
		...
		for (jj = 0; jj < (int)VecAclnDotInfo.size(); jj++)
		{
			if (tmpDevInfo.dot1.dot_id == VecAclnDotInfo[jj].acln_dot_id)
			{
				...
				//  				sprintf(m_LogString,"[yjj]线路首端名称["INT_REC_FMT"][%f]",tmpDevInfo.dot1.dot_id,tmpDevInfo.dot1.dot_name);
				//  				MessShow(m_LogString,LTERROR);
			}
			else if (tmpDevInfo.dot2.dot_id == VecAclnDotInfo[jj].acln_dot_id)
			{
				...
				// 				sprintf(m_LogString,"[yjj]线路末端名称["INT_REC_FMT"][%f]",tmpDevInfo.dot2.dot_id,tmpDevInfo.dot2.dot_name);
				//  				MessShow(m_LogString,LTERROR);
			}
			if (nFlag >= 2)
			{
...
int CLfoAnaObj::AddLfoIntface(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (i = 0; i < (int)VecLfoInt.size(); i++)
	{
		...
		sprintf(m_LogString, "断面名称[%s][%ld]", tmpLfoDevInfo.dev.dev_name, tmpLfoDevInfo.dev.dev_id);
		MessShow(m_LogString, LTINFO);
		VecMonDev.push_back(tmpLfoDevInfo);
	}
#endif
...
int CLfoAnaObj::AddLfoIntLine(vector<LFO_INTLINE_INFO>& VecINTLINE)
{
	...
	for (i = 0; i < (int)VecLfoIntline.size(); i++)
	{
		...
		sprintf(m_LogString, "断面名称[%s][%ld]支路名称[%s][%ld],测点厂站[%ld]", tmpLfoINTlineInfo.inf_name, tmpLfoINTlineInfo.inf_no, tmpLfoINTlineInfo.line_name, tmpLfoINTlineInfo.line_no, tmpLfoINTlineInfo.stat_no);
		MessShow(m_LogString, LTINFO);
...
int CLfoAnaObj::UpdateMonGnDisp(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (ii = 0; ii < (int)VecMonDev.size(); ii++)
	{
		...
		if (ii < m_LfoPara.disp_num)
		{
			...
			sprintf(m_LogString, "[%d][%s]需要显示,功率波动[%f]", VecMonDev[ii].dev.dev_idx, VecMonDev[ii].dev.dev_name, VecMonDev[ii].curve.max_ampl);
			MessShow(m_LogString);
		}
		else
		{
...
int CLfoAnaObj::UpdateMonTrTable(vector<LFO_DEV_INFO>& VecMonDev)
{
	...
	for (ii = 0; ii < (int)VecMonDev.size(); ii++)
	{
		...
		// 		tmpMonDev.max_ampl_rat = VecMonDev[ii].curve.max_ampl_rat;
				//tmpMonDev.slct_dot_id = VecMonDev[ii].slct_dot_id;
		//   		sprintf(m_LogString,"更新LFO变压器表,[%s]状态[%d],功率波动[%f]",VecMonDev[ii].dev.dev_name,VecMonDev[ii].stat,tmpMonDev.max_min_dif);
		//   		MessShow(m_LogString,LTINFO);
...
int CLfoAnaObj::UpdateLfoMonPara(LFO_SYS_INFO& MonSys)
{
	...
	INT_REC record_id = PODB_GetDevIdByKeyID(LFO_PARA_NO_TAB, 2);
	sprintf(m_LogString, "获取LFO参数表第2条记录的记录号为["INT_REC_FMT"],系统状态[%d],振荡源分析[%s]]", record_id, MonSys.stat, tmpLfoParaMon.lfos_des);
	MessShow(m_LogString, LTINFO);
...
// 	}
	...
	dtime = float(1 / dtime);
	sprintf(m_LogString, "start_time = %d,end_time = %d,dtime = %f", tmpLfoTime.start_time, tmpLfoTime.end_time, dtime);
	MessShow(m_LogString, LTINFO);
	...
	for (int ii = 0; ii < m_VecIntline.size(); ii++)
	{
		sprintf(m_LogString, "断面id[%ld],支路ID[%ld]", tmpLfoInf.dev.dev_id, m_VecIntline[ii].line_no);
		MessShow(m_LogString, LTINFO);
	...
	sprintf(m_LogString, "曲线:发电机[%s]["INT_REC_FMT"][%d]交流线路[%s]["INT_REC_FMT"][%d]变压器[%s]["INT_REC_FMT"][%d]time[%d~%d]", tmpLfoGen.dev.dev_name, tmpLfoGen.dev.dev_id, tmpLfoGen.stat,
		tmpLfoLine.dev.dev_name, tmpLfoLine.dev.dev_id, tmpLfoLine.stat, tmpLfoTr.dev.dev_name, tmpLfoTr.dev.dev_id, tmpLfoTr.stat, MonSys.start_time, MonSys.end_time);
	MessShow(m_LogString, LTINFO);
...
// 		MessShow(m_LogString,LTINFO);
		...
		{//本轮结束时间-案例开始时间<60 读取从案例开头到本轮结束时间的数据
			...
			sprintf(m_LogString, "未超过60秒,读取数据文件[%s],起始[%d],窗口[%d],交叠[0]", m_LfoPara.datafile_name, pos, tmpLfoTime.end_time - m_LfoPara.file_start_time);
			MessShow(m_LogString, LTINFO);
...
// 	UpdateLfoCurve(tmpLfoInf);
	...
	if (b_curve == true)
	{
		PODB_TableClear(LPO_RT_CURVE_NO);
...
int CLfoAnaObj::SendWarnMessage(LFO_SYS_INFO& MonSys, int type, int end_time, int hold_time, int if_lfo)
{
	...
	if (if_lfo == 0)//原有的功率波动告警流程
	{
		if (type == LFO_UNSAVE_WARN)
		{
			...
			// 				}
			// 			}
			sprintf(m_LogString, "功率波动告警:host[%s],fac_id["INT_REC_FMT"],desc[%s]", hostname, m_LfoPara.lfo_fac_id, tmpstring.c_str());
			MessShow(m_LogString, LTINFO);
		}
		...
		else if (type == LFO_SAVE_WARN)
		{
			...
			sprintf(statdes, " 功率波动告警结束,持续时间%s。", temp_time_str);
			...
			sprintf(m_LogString, "功率波动告警结束:host[%s],fac_id["INT_REC_FMT"],desc[%s]", hostname, m_LfoPara.lfo_fac_id, tmpstring.c_str());
			MessShow(m_LogString, LTINFO);
		}
	}
	else if (if_lfo == 1)
	{
		if (type == LFO_UNSAVE_WARN)
		{
			...
			sprintf(m_LogString, "低频振荡告警:host[%s],fac_id["INT_REC_FMT"],desc[%s]", hostname, m_LfoPara.lfo_fac_id, tmpstring.c_str());
			MessShow(m_LogString, LTINFO);
		}
		else if (type == LFO_SAVE_WARN)
		{
			...
			sprintf(m_LogString, "低频振荡结束:host[%s],fac_id["INT_REC_FMT"],desc[%s]", hostname, m_LfoPara.lfo_fac_id, tmpstring.c_str());
			MessShow(m_LogString, LTINFO);
		}
	...
	{
		tmpMsg.seq_warn_message[0].warn_type = WAMS_POW_WARN_TYPE;
	}
#else	
...
int CLfoAnaObj::SendWarnToIA(LFO_SYS_INFO& MonSys, const int mode)
{
	...
	for (ii = 0; ii < (int)tmpVecDev.size(); ii++)
	{
		sprintf(m_LogString, "%d,"INT_REC_FMT","INT_REC_FMT",%d,%s,%s,%f,%f,%f,%f", ii + 1,
			tmpVecDev[ii].osci_id, tmpVecDev[ii].key_id, tmpVecDev[ii].group_num, tmpVecDev[ii].group_name,
...
int CLfoAnaObj::SetEmsStat(LFO_SYS_INFO& MonSys)
{
	...
	if (key <= 0)
	{
		sprintf(m_LogString, "EMS触发keyid错误["INT_REC_FMT"]", key);
		MessShow(m_LogString, LTERROR);
		return -1;
	}
...
int CLfoAnaObj::ShowWamsCurve(LFO_SYS_INFO& MonSys)
{
	...
	{
		DevPowCol = PODB_GetFieldNoByName(GN_DEVICE_NO_TAB, P_VAL, 70);
	}
	else if (table_no == ACLN_DOT_NO_TAB)
	{
		DevPowCol = PODB_GetFieldNoByName(ACLN_DOT_NO_TAB, P_VAL, 30);
	}
	else if (table_no == TRWD_DEVICE_NO_TAB)
	{
		DevPowCol = PODB_GetFieldNoByName(TRWD_DEVICE_NO_TAB, P_VAL, 50);
	}
	else
	{
		sprintf(m_LogString, "表号错误[%d],dev["INT_REC_FMT"]", table_no, MonSys.lead_dev.dev.dev_id);
		MessShow(m_LogString, LTERROR);
		return -1;
	}
#endif
	...
	sprintf(tmpString, "WamsCurve -send -devname %s%s -app_key_id %d/"INT_REC_FMT"/%d &",
		MonSys.lead_dev.dev.dot_fac_name, MonSys.lead_dev.dev.dot_dev_name, g_appNo, MonSys.lead_dev.dev.dot_id, DevPowCol
	);
...
int CLfoAnaBase::LocateLfoSource(vector<LFO_DEV_INFO>& VecMonDev, LFO_SYS_INFO& MonSys)
{
	...
	INT_REC StartTime = m_MonSys.start_time;
	INT_REC EndTime = m_MonSys.end_time;
	int ordernum;
	int nNeedNum;
	nNeedNum = (EndTime - StartTime) * MAX_FRA_PER_SEC;
	int bak_kkk;
...
// 							tmpstring = m_df.m_DataHead.vec_devid_s[djj];
							...
							if (tmpdev_id == tmpLfoLn.dev.dev_id)
							{
								...
								if (nColNum == ordernum)
								{
									...
									{
										sprintf(m_LogString, "[%s]["INT_REC_FMT"]读取数据错误[%s]",
											tmpstring.c_str(), tmpdev_id, m_df.GetLastError());
										MessShow(m_LogString, LTDEBUG);
									}
									else
									{
				...
				{
					delt_t = float(1.0 / m_df.m_DataHead.time_sample); //单位s
					if (kk == 0)
					{
				...
				for (int k = 0; k < (int)tmpVecCase.size(); k++)
				{
					if (tmpLfoLn.dev.dev_id == tmpVecCase[k].dev.dev_id)
					{
						...
						{
							sprintf(m_LogString, "Prony算法未分析出支路[%s]角速度符合要求的振荡模式!", tmpLfoLn.dev.dev_name);
							MessShow(m_LogString);
							//	prony_flag=1;
						}
					}
	...
	{
		sprintf(m_LogString, "没有振荡告警设备，无需振荡源定位!");
		MessShow(m_LogString);
		// 		SAFE_DELETE_ARRAY(pData);
		if (pAllObjMode) { free(pAllObjMode);   pAllObjMode = NULL; }
		if (pAllMode) { free(pAllMode);   pAllMode = NULL; }
			...
			for (int k = 0; k < (int)tmpVecCase.size(); k++)
			{
				if (tmpLfoGn.dev.dev_id == tmpVecCase[k].dev.dev_id)
				{
					...
					else
					{
						...
						temp_V_phas = 0.0;
						prony_flag++;
						sprintf(m_LogString, "Prony算法未分析出机组[%s]电压对数符合要求的振荡模式!", tmpLfoGn.dev.dev_name);
						MessShow(m_LogString);
					}
				}
			}
...
//				  	 tmpLfoGn.v2d_data[kk]=tmpLfoGn.v2d_data[kk-1];//近似取值										
			...
			for (k = 0; k < (int)tmpVecCase.size(); k++)
			{
				if (tmpLfoGn.dev.dev_id == tmpVecCase[k].dev.dev_id)
				{
					if (!tmpVecCase[k].vec_mode.empty())
					{
						...
						if (nsamemode == 0)
						{
							...
							temp_A_phas = 0.0;
							prony_flag++;
							sprintf(m_LogString, "Prony算法分析出机组[%s]角速度振荡模式与主导模式不同调!", tmpLfoGn.dev.dev_name);
							MessShow(m_LogString);
						}
					}
					else
					{
						...
						temp_A_phas = 0.0;
						prony_flag++;
						sprintf(m_LogString, "Prony算法未分析出机组[%s]角速度符合要求的振荡模式!", tmpLfoGn.dev.dev_name);
						MessShow(m_LogString);
					}
				}
			}
				...
				if (tmpLfoGn.dev.dev_id == tmpVecCase[k].dev.dev_id)
				{
					if (!tmpVecCase[k].vec_mode.empty())
					{
						...
						if (nsamemode == 0)
						{
							...
							temp_p_phas = 0.0;
							prony_flag++;
							sprintf(m_LogString, "Prony算法分析出机组[%s]有功功率振荡模式与主导模式不同调!", tmpLfoGn.dev.dev_name);
							MessShow(m_LogString);
						}
						//		prony_flag = 0;
					}
					else
					{
						...
						temp_p_phas = 0.0;
						prony_flag++;
						sprintf(m_LogString, "Prony算法未分析出机组[%s]有功功率符合要求的振荡模式!", tmpLfoGn.dev.dev_name);
						MessShow(m_LogString);
					}
			...
			MessShow(m_LogString);
			sprintf(m_LogString, "机组[%s]耗散功率=%f", tmpLfoGn.dev.dev_name, temp_pow);
			MessShow(m_LogString);
		}
		...
		for (ii = 0; ii < pSltMode[0].nPartLn; ii++)
		{
				...
				if (tmpLfoLn.dev.dev_id == tmpVecCase[k].dev.dev_id)
				{
					if (!tmpVecCase[k].vec_mode.empty())
					{
						...
						if (nsamemode == 0)
						{
							...
							temp_V_phas = 0.0;
							prony_flag++;
							sprintf(m_LogString, "Prony算法分析出线路[%s]电压振荡模式与主导模式不同!", tmpLfoGn.dev.dev_name);
							MessShow(m_LogString);
						}
					}
					else
					{
						...
						temp_V_phas = 0.0;
						prony_flag++;
						sprintf(m_LogString, "Prony算法未分析出线路[%s]电压幅值符合要求的振荡模式!", tmpLfoLn.dev.dev_name);
						MessShow(m_LogString);
					}
				}
			}
			...
			else
			{
				...
				temp_A_phas = 0.0;
				prony_flag++;
				sprintf(m_LogString, "未获取线路[%s]角速度符合要求的振荡模式!", tmpLfoLn.dev.dev_name);
				MessShow(m_LogString);
			}
				...
				if (tmpLfoLn.dev.dev_id == tmpVecCase[k].dev.dev_id)
				{
					if (!tmpVecCase[k].vec_mode.empty())
					{
						...
						if (nsamemode == 0)
						{
							...
							temp_q_phas = 0.0;
							prony_flag++;
							sprintf(m_LogString, "Prony算法分析出线路[%s]无功功率振荡模式与主导模式不同!", tmpLfoGn.dev.dev_name);
							MessShow(m_LogString);
						}
					}
					else
					{
						...
						temp_q_phas = 0.0;
						prony_flag++;
						sprintf(m_LogString, "Prony算法未分析出线路[%s]无功功率符合要求的振荡模式!", tmpLfoLn.dev.dev_name);
						MessShow(m_LogString);
					}
				}
			}
				...
				if (tmpLfoLn.dev.dev_id == tmpVecCase[k].dev.dev_id)
				{
					if (!tmpVecCase[k].vec_mode.empty())
					{
						...
						if (nsamemode == 0)
						{
							...
							temp_p_phas = 0.0;
							prony_flag++;
							sprintf(m_LogString, "Prony算法分析出线路[%s]有功功率振荡模式与主导模式不同!", tmpLfoGn.dev.dev_name);
							MessShow(m_LogString);
						}
					}
					else
			...
			sprintf(m_LogString, "线路[%s]所属群[%d],耗散功率=%f", tmpLfoLn.dev.dev_name, pSltMode[0].PartLn[bak_kkk].clus, pSltMode[0].PartLn[bak_kkk].pow);
			MessShow(m_LogString);
		}
...
// 							tmpstring = m_m_df.m_DataHead.vec_devid_s[djj];
				...
				if (tmpLfoTr.dev.dev_id == tmpVecCase[k].dev.dev_id)
				{
					if (!tmpVecCase[k].vec_mode.empty())
					{
						...
						if (nsamemode == 0)
						{
							...
							temp_V_phas = 0.0;
							prony_flag++;
							sprintf(m_LogString, "Prony算法分析出变压器[%s]电压幅值振荡模式与主导模式不同!", tmpLfoGn.dev.dev_name);
							MessShow(m_LogString);
						}
					}
					else
					{
						...
						temp_V_phas = 0.0;
						prony_flag++;
						sprintf(m_LogString, "Prony算法未分析出变压器[%s]电压幅值符合要求的振荡模式!", tmpLfoTr.dev.dev_name);
						MessShow(m_LogString);
					}
				}
			}
			...
			else
			{
				...
				temp_A_phas = 0.0;
				prony_flag++;
				sprintf(m_LogString, "未获取变压器[%s]角速度符合要求的振荡模式!", tmpLfoTr.dev.dev_name);
				MessShow(m_LogString);
			}
			ClearLfoPronyInfo1(tmpVecCase);
...
// 							tmpstring = m_m_df.m_DataHead.vec_devid_s[djj];
		...
		int source_gnflag[para_nGnMaxMem];
		int source_gnflag1[para_nGnMaxMem];
		int source_lnflag[para_nLnMaxMem];
		int source_lnflag1[para_nLnMaxMem];
		int source_trflag[para_nTrMaxMem];
		int source_trflag1[para_nTrMaxMem];
...
int CLfoAnaObj::SampleHisEve(LFO_CASE_INFO& LfoCase)
{
	...
	tmpVecTrigger.push_back(tmpTrigger);
	sprintf(m_LogString, "LPO功率波动事件表采样,记录数[%d],occur_time=%d,eve_id=%d", tmpVecTrigger.size(), tmpTrigger.occur_time, tmpHisEve.eve_id);
	MessShow(m_LogString, LTINFO);
			...
			if (m_nSltMode > 0 && m_pSltMode[0].nPartFc > 0)
			{
				for (kk = 0; kk < m_pSltMode[0].nPartFc; kk++)
				{
					if (m_pFcAna[m_nFcProDat].fac_id == m_pSltMode[0].PartFc[kk].obj_id)
					{
						...
						sprintf(m_LogString, "[%s][%ld]电压最大摆动[%f],频率[%f],初相角[%f],所属群[%d]", m_VecDev[ii].dev.dev_name, m_pFcAna[m_nFcProDat].fac_id, m_pFcAna[m_nFcProDat].lfo_v_ampl, m_pFcAna[m_nFcProDat].freq, m_pFcAna[m_nFcProDat].phas, m_pFcAna[m_nFcProDat].lfo_clus);
						MessShow(m_LogString);
						break;
					}
				}
		...
		if (m_pSltMode[0].nPartFc > 0)
		{
			for (ii = 0; ii < m_pSltMode[0].nPartFc; ii++)
			{
				...
				for (jj = 0; jj < m_nFcProDat; jj++)
				{
					...
					{
						sprintf(m_LogString, "厂站[%ld]lfo_v_ampl=%f,MaxVoltAmpl=%f!", m_pFcAna[jj].fac_id, m_pFcAna[jj].lfo_v_ampl, MaxVoltAmpl);
						MessShow(m_LogString);

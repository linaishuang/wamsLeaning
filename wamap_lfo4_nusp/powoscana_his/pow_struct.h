...
// 	INT_REC freq_per_id;  // 频率允许偏移组
	...
	// 
	//WT_DOT_INFO select_dot;  // 选择端点
}WT_DEV_INFO;
////////////////////////////////////////////////
//以上来自wamtoolstruct.h 以下来自lfo_struct.h
////////////////////////////////////////////////
// 设备基本信息
...
#define LFO_GN_TAB_MONWRITE_FLD "stat2,stat_val2,lead_ampl2,lead_freq2,lead_damp2,lead_phas2,lead_real2,lead_engy2,lead_fail2,aver_val2,max_val2,max_val_t2,min_val2,min_val_t2,max_min_dif2,fact2,fact_sts2,lead_gen_pow,clus,is_lfo_source"
...
// 	float lfo_end_val;	/* LFO后均值2 */
	...
	long lfo_max_val_t;	/* 最大值时间2 */
	float lfo_min_val;	/* 最小值2 */
	long lfo_min_val_t;	/* 最小值时间2 */
...
// 	float aver_val2;	/* 均值2 */
	int if_lfo2;	/* 是否出现振荡2 */
	long lfo_bgn_t2;	/* LFO发生时间2 */
	long lfo_end_t2;	/* LFO消失时间2 */
	char lfo_time2[20];	/* 持续时间2 */
	float lfo_max_val2;	/* LFO最大值2 */
	long lfo_max_val_t2;	/* LFO最大值时间2 */
	float lfo_min_val2;	/* LFO最小值2 */
	long lfo_min_val_t2;	/* LFO最小值时间2 */
	char intline_des2[256]; /* 断面组成描述2 */
	unsigned char lfo_is_disp2;	/* 是否显示 */
}LFO_INF_TAB_WRITE;
...
typedef struct type_POW_WDS_WRITE
{
	...
	long eve_id;	/* 事件ID */
	long win_id;	/* 窗口号 */
	long analy_bgn;	/* 分析起始时间 */
	long analy_end;	/* 分析截止时间 */
	int is_show;	/* 是否显示 */
	float rphas;	/* 初相 */
	long lfodev_id;	/* 振荡设备ID */
	long fac_id;	/* 厂站ID */
	int dev_type;	/* 设备类型 */
	float freq;	/* 频率 */
	float ampl;	/* 振幅 */
	...
	float lfo_max_val;	/* LFO最大值 */
	long lfo_max_val_t;	/* LFO最大值时间 */
	float lfo_min_val;	/* LFO最小值 */
	long lfo_min_val_t;	/* LFO最小值时间 */

...
#include <stdio.h>
//#define calldgelss CALLDGELSS
//extern "C" void calldgelss_(int *m,int *n,double *A,double *B,double *X);

// extern  "C" void  c_dgesvd();
...
void   CPronyAnalysis::PronyAnalysis(PRONY_CALC_PARA calcpara, int oldnum, double dataxp[], double datatp[], int &outmodenum, PRONY_MODE_CHARA *outmode, double &snr,int &actrank)
{
//	if (calcpara.show_log >= 1)
//	{
//  		sprintf(m_LogStr,"进入Prony分析函数PronyAnalysis");
//  		MessShow(m_LogStr);
//	}
	if ( !dataxp || !datatp || oldnum <= 5 )
	{
		sprintf(m_LogStr,"数据错误");
		MessShow(m_LogStr);
		return ;
	}
    ...
    int    *modp = NULL;
	//double *dataxp=NULL,*datatp=NULL;                // data_x[],data_t[]：原始数据；dataxp[],datatp：预处理后数据；ppx：Prony分析后拟合数据
	...
	PRONY_MODE_CHARA *mode=NULL;	
//	PRONY_MODE_CHARA *m_mode=NULL;
...
// 	dataxp = new double[oldnum];
// 	datatp = new double[oldnum];
// 	memset((void*)dataxp, 0 , oldnum*sizeof(double));
// 	memset((void*)datatp, 0 , oldnum*sizeof(double));
	...
	if(nn >= 5)
	{
		tt = datatp[4]- datatp[3];
	}
	else
	{
		modenum = -2;
		sprintf(m_LogStr,"数据点数过少[%d]<5,退出",nn);
		MessShow(m_LogStr);
		return ;
	}

	if (nn > PRONY_DATA_MAX)
	{
		sprintf(m_LogStr,"数据点数越限[%d]>[%d],取前[%d]个",nn,PRONY_DATA_MAX,PRONY_DATA_MAX);
		MessShow(m_LogStr);
		nn = PRONY_DATA_MAX;
	}
	//一阶惯性滤波处理	
// 	datayp=(double *)calloc(oldnum,sizeof(double));
// 	memset((void *)datayp,  0, oldnum * sizeof(double));
// 	if (calcpara.is_filter == 1)
// 	{
// 	sprintf(m_LogStr,"进行一阶惯性滤波处理!");
// 	MessShow(m_LogStr);	
// 		if(int ret = Euler_ValFilter(nn, dataxp, datatp, datayp) > 0)
// 		{
// 			sprintf(m_LogStr,"一阶惯性滤波处理异常!");
// 			MessShow(m_LogStr);			
// 		}
// 		for (k=0; k<nn; k++)
// 		{
...
// 	nn = oldnum;
	...
	// 原始波形数值减去直流分量值
	// 计算去直流后原波形能量值
	double engorg = 0.0f,engmax = 0.0f;
	for ( i =1; i<nn; i++)
	{
		engorg += fabs(fabs(dataxp[i-1]-dcvalue)+fabs(dataxp[i]-dcvalue)) * tt / 2.0f;
	}
	...
	do 
	{
		...
		}while ((snr1 < calcpara.snr_min) && (intn < calcpara.max_iter));
		...
		if (snr1 < calcpara.snr_min)
		{
			...
			//if( m_mode != NULL)  { free(m_mode);	m_mode=NULL; }
			modenum = 0;
			sprintf(m_LogStr,"snr1=%f < calcpara.snr_min = %f,intn=%d,np=%d",snr1,calcpara.snr_min,intn,np);
			MessShow(m_LogStr);
			return;
		}
...
//解决Prony分析出模式数越限的问题 20160331
		mode=(PRONY_MODE_CHARA *)calloc(np,sizeof(PRONY_MODE_CHARA));
//		m_mode=(PRONY_MODE_CHARA *)calloc(np,sizeof(PRONY_MODE_CHARA));
		memset((void*)mode,0,np*sizeof(PRONY_MODE_CHARA));
//		memset((void*)m_mode,0,np*sizeof(PRONY_MODE_CHARA));
...
// 						}
// 						获取模式能量最大值
						if (mode[mod_num].engy > engmax && mode[mod_num].freq != 0.0f)
						{
							engmax = mode[mod_num].engy;
						}
						mod_num=mod_num+1;
					}
					else
...
// 						获取模式能量最大值
		...
		if (engmax > calcpara.eng_time*engorg)
		{
			// 最大模式能量值大于原始波形能量的engtime倍,阶数减1迭代次数加1重新计算
			np = np -1;
			intn = intn + 1;
	...
	} while ((engmax>calcpara.eng_time*engorg) && (intn < calcpara.max_iter));
...
//		if( m_mode != NULL) { free(m_mode);	m_mode=NULL; }
		modenum = -3;
		snr = 0.0f;
		sprintf(m_LogStr,"engmax=%f > calcpara.eng_time*engorg = %f*%f",engmax,calcpara.eng_time,engorg);
		MessShow(m_LogStr);
		return;
	}
	////////////////////////////////////////////////////////////////////////////////
	...
	if (calcpara.show_log >= 1)
	{
		...
		MessShow(m_LogStr);
		sprintf(m_LogStr,"freq_min=%f,freq_max=%f,minAbsAmpl=%f,stor_lim_ampl=%f,stor_lim_damp=%f,eng_time=%f,max_iter=%d",
			calcpara.freq_min,calcpara.freq_max,calcpara.minAbsAmpl,calcpara.stor_lim_ampl,calcpara.stor_lim_damp,calcpara.eng_time,calcpara.max_iter);
		MessShow(m_LogStr);
//		if (calcpara.show_log >=2 )
//		{
//			for (int ii =0; ii<nn; ii++)
//			{
//				sprintf(m_LogStr,"%.4f    %.6f",datatp[ii],dataxp[ii]);
//				MessShow(m_LogStr);
//			}
//		}
		sprintf(m_LogStr,"数据点数:%d,模式数:%d,信噪比:%f,阶数:%d,直流:%f,原始能量:%f,迭代:%d",oldnum,mod_num, snr,actrank,dcvalue,engorg,intn);
		MessShow(m_LogStr);
...
// 		delete []curve;
	...
	if (mod_num>PRONY_MAX_MODE)
	{
		mod_num = PRONY_MAX_MODE;
	}
	outmodenum = 0;
	for ( k=0; k<mod_num; k++ )
	{
		outmode[outmodenum].damp = mode[k].damp;
		outmode[outmodenum].freq = mode[k].freq;  
		outmode[outmodenum].ampl = mode[k].ampl;
		outmode[outmodenum].phas = mode[k].phas;
		outmode[outmodenum].real = mode[k].real;   
		outmode[outmodenum].imag = mode[k].imag;       
		outmode[outmodenum].zphs = mode[k].zphs;
		outmode[outmodenum].zamp = mode[k].zamp;
		outmode[outmodenum].brel = mode[k].brel;
		outmode[outmodenum].bimg = mode[k].bimg;
		outmode[outmodenum].engy = mode[k].engy;
		outmodenum = outmodenum + 1;
	}
	if( modp != NULL) { free(modp);	modp=NULL; }
	if( br != NULL)  { free(br);	br=NULL; }
	if( bi != NULL)  { free(bi);	bi=NULL; }
...
//	if (calcpara.show_log >= 1)
//	{
//  		sprintf(m_LogStr,"退出Prony分析函数PronyAnalysis");
//  		MessShow(m_LogStr);
//	}
	return;
}
...
//	sprintf(m_LogStr,"calldgelss开始,m=%d,n=%d",mm,nn);
//	MessShow(m_LogStr);
//	calldgelss_(&mm,&nn,ca,cb,xx);
//	sprintf(m_LogStr,"calldgelss结束");
//	MessShow(m_LogStr);
	for (i=0;i<=n-1;i++)
	{
	   xr[i]=xx[i];
...
int CPronyAnalysis::CheckPara( PRONY_CALC_PARA &calcpara )
{
	...
	{
		calcpara.snr_min = 30;
		sprintf(m_LogStr,"最小信噪比限值错误,置为[%.3f]",calcpara.snr_min);
		MessShow(m_LogStr);
	}
	if (calcpara.eng_time < 1.0)
	{
		calcpara.eng_time = 2.0f;
		sprintf(m_LogStr,"最大能量倍数错误,置为[%.3f]",calcpara.eng_time);
		MessShow(m_LogStr);
	}
	if (calcpara.max_iter < 1)
	{
		calcpara.max_iter = 4;
		sprintf(m_LogStr,"最大迭代数错误,置为[%d]",calcpara.max_iter);
		MessShow(m_LogStr);
	}
	return nflag;
}
// int CPronyAnalysis::Euler_ValFilter(int np, double xp[], double tp[], double yp[])
// {

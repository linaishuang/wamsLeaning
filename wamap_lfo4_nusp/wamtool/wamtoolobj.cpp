...
void CWamToolObj::InitPara()
{
	...
	m_maindir = m_homedir + "/data/wam_poa";
...
int CWamToolObj::WamToolMain()
{
	...
	if (m_WTPara.IsFacUpdate == 1)
	{
		...
		// 置电压等级标志,是否新增标志
		SetAddFlag(m_VecFacInfo, LFO_FAC_NO_TAB);
		// WriteDevFile(m_VecFacInfo, LFO_FAC_NO_TAB);  //lns_20211026  jibei
		// 增加厂站
		WriteDevNew(m_VecFacInfo, LFO_FAC_NO_TAB);
	}
	// LFO发电机表,置LFO发电机标志
	if (m_WTPara.IsGenUpdate == 1)
	{
		...
		// 置电压等级标志,是否新增标志
		SetAddFlag(m_VecGenInfo, LFO_GN_NO_TAB);
		// WriteDevFile(m_VecGenInfo, LFO_GN_NO_TAB);  //lns_20211026  jibei
		// 增加发电机
		WriteDevNew(m_VecGenInfo, LFO_GN_NO_TAB);
	}
	...
	if (m_WTPara.IsAclnUpdate == 1)
	{
		...
		// 置电压等级标志,是否新增标志
		SetAddFlag(m_VecAclnInfo, LFO_ACLN_NO_TAB);
		// WriteDevFile(m_VecAclnInfo, LFO_ACLN_NO_TAB); //lns_20211026  jibei
		// 增加交流线路
		WriteDevNew(m_VecAclnInfo, LFO_ACLN_NO_TAB);
	}
	...
	if (m_WTPara.IsTrUpdate == 1)
	{
		...
		// 置电压等级标志,是否新增标志
		SetAddFlag(m_VecTrInfo, LFO_TR_NO_TAB);
		// WriteDevFile(m_VecTrInfo, LFO_TR_NO_TAB); //lns_20211026  jibei
		// 增加交流线路
		WriteDevNew(m_VecTrInfo, LFO_TR_NO_TAB);
	}
	...
	if (m_WTPara.IsFacUpdate == 1 || m_WTPara.IsGenUpdate == 1 ||
		m_WTPara.IsAclnUpdate == 1 || m_WTPara.IsTrUpdate == 1)
	{
...
int CWamToolObj::InitProject()
{
	...
	//if (m_WTPara.IsFacUpdate == 1)
	{
		if (InitFacInfo(m_VecFacInfo, VecFacTab, VecAreaTab, VecVLTypeTab) < 0)
		{
			return -1;
		}
		// 置测点遥测标志
		SetYcFlag(m_VecFacInfo, VecFesYcTab, LFO_FAC_NO_TAB);
	}
	...
	//if (m_WTPara.IsGenUpdate == 1)
	{
		...
		SetYcFlag(m_VecGenInfo, VecFesYcTab, LFO_GN_NO_TAB);
	}
	...
	//if (m_WTPara.IsAclnUpdate == 1)
	{
		...
		SetYcFlag(m_VecAclnInfo, VecFesYcTab, LFO_ACLN_NO_TAB);
	}
	...
	//if (m_WTPara.IsTrUpdate == 1)
	{
		...
		SetYcFlag(m_VecTrInfo, VecFesYcTab, LFO_TR_NO_TAB);
	}
...
#ifdef _NUSP
	INT_REC MAX_VFIdxTemp = pInfo[0].bus_idx;
	for (ii = 0; ii < nInfo; ii++)
	{
		if (MAX_VFIdxTemp < pInfo[ii].bus_idx)
		{
			MAX_VFIdxTemp = pInfo[ii].bus_idx;
		}
	}
	m_WTPara.MAX_VFOld_idx = MAX_VFIdxTemp;
#endif // _NUSP
...
int CWamToolObj::InitFacInfo(vector<WT_DEV_INFO>& VecDevInfo, vector<FAC_TAB_CRE>& VecFacTab, vector<AREA_TAB_CRE>& VecAreaTab, vector<VL_TYPE_TAB_CRE>& VecVLTypeTab)
{
	if (!VecDevInfo.empty())
	{
	...
	for (ii = 0; ii < (int)VecFacTab.size(); ii++)
	{
		...
		tmpDevInfo.dev_type = VecFacTab[ii].fac_type;
	...
	for (ii = 0; ii < (int)VecDevInfo.size(); ii++)
	{
		...
		{
			if (VecDevInfo[ii].fac_id == VecBusTab[jj].fac_id)
			{
				tmpVecBus.push_back(VecBusTab[jj]);
			}
		}

		ZERO_INT_REC(tmpBusID);
		tmpVltyID = 0;
		tmpvbase = 0.0f;
		sprintf(tmpBusName, "%s", "未匹配");
		if (tmpVecBus.empty())
		{
			sprintf(m_LogString, "["INT_REC_FMT"][%s]未找到对应母线", INT_REC_VAL(VecDevInfo[ii].dev_id), VecDevInfo[ii].dev_name);
			MessShow(m_LogString, LTERROR);
		}
		else
		{
			for (jj = 0; jj < (int)tmpVecBus.size(); jj++)
			{
				nFlag = 0;
				for (kk = 0; kk < (int)VecVLTypeTab.size(); kk++)
				{
					if (tmpVecBus[jj].vlty_id == VecVLTypeTab[kk].vlty_id)
					{
						// 记录最高电压等级母线ID和电压等级
						if (VecVLTypeTab[kk].vbase > tmpvbase)
						{
							tmpBusID = tmpVecBus[jj].bs_id;
							tmpVltyID = VecVLTypeTab[kk].vlty_id;
							tmpvbase = VecVLTypeTab[kk].vbase;
							sprintf(tmpBusName, "%s", tmpVecBus[jj].bs_name);
						}
						nFlag = 1;
						break;
					}
				...
				if (nFlag == 0)
				{
					sprintf(m_LogString, "母线["INT_REC_FMT"][%s]未找到对应电压等级[%d]", INT_REC_VAL(tmpVecBus[jj].bs_id), tmpVecBus[jj].bs_name, tmpVecBus[jj].vlty_id);
					MessShow(m_LogString, LTERROR);
				}
			}
			if (!VALID_INT_REC(tmpBusID))
			{
				sprintf(m_LogString, "["INT_REC_FMT"][%s]未找到最高电压等级母线", INT_REC_VAL(VecDevInfo[ii].dev_id), VecDevInfo[ii].dev_name);
				MessShow(m_LogString, LTERROR);
			}
		}

		// 厂站最高电压等级母线
		VecDevInfo[ii].dot1.dot_id = tmpBusID;
		VecDevInfo[ii].dot1.area_id = VecFacTab[ii].area_id;
		VecDevInfo[ii].dot1.fac_id = VecFacTab[ii].fac_id;
		VecDevInfo[ii].dot1.vlty_id = tmpVltyID;
		VecDevInfo[ii].dot1.vbase = tmpvbase;
		VecDevInfo[ii].maxvlty_id = tmpVltyID;
		VecDevInfo[ii].maxvbase = tmpvbase;
		sprintf(VecDevInfo[ii].dot1.dot_name, "%s", tmpBusName);
		sprintf(VecDevInfo[ii].dot1.fac_name, "%s", VecDevInfo[ii].fac_name);
		sprintf(VecDevInfo[ii].dot1.area_name, "%s", VecDevInfo[ii].area_name);
	}
	...
	for (ii = 0; ii < (int)VecDevInfo.size(); ii++)
	{
		...
		if (nFlag == 0)
		{
			sprintf(m_LogString, "["INT_REC_FMT"][%s]未找到对应频率测点["INT_REC_FMT"]", INT_REC_VAL(VecDevInfo[ii].dev_id), VecDevInfo[ii].dev_name, INT_REC_VAL(VecDevInfo[ii].fac_id));
			MessShow(m_LogString, LTERROR);
		}
	}
#endif
...
int CWamToolObj::GetLfoFac(vector<WT_DEV_INFO>& VecDevInfo)
{
	...
	{
		sprintf(m_LogString, "读取LFO厂站表失败");
		MessShow(m_LogString, LTERROR);
		return -1;
	}
	...
	for (ii = 0; ii < (int)VecDevInfo.size(); ii++)
	{
		for (jj = 0; jj < nInfo; jj++)
		{
			if (VecDevInfo[ii].dev_id == pInfo[jj].fac_id)
			{
				VecDevInfo[ii].is_old = 1;
				break;
			}
...
#ifdef _NUSP
	INT_REC MAX_FacIdxTemp = pInfo[0].fac_idx;
	for (ii = 0; ii < nInfo; ii++)
	{
		if (MAX_FacIdxTemp < pInfo[ii].fac_idx)
		{
			MAX_FacIdxTemp = pInfo[ii].fac_idx;
		}
	}
	m_WTPara.MAX_FacOld_idx = MAX_FacIdxTemp;
#endif // _NUSP
...
int CWamToolObj::GetLfoGen(vector<WT_DEV_INFO>& VecDevInfo)
{
	...
	{
		sprintf(m_LogString, "读取LFO发电机表失败");
		MessShow(m_LogString, LTERROR);
		return -1;
	}
...
#ifdef _NUSP
	INT_REC MAX_GenIdxTemp = pInfo[0].gn_idx;
	for (ii = 0; ii < nInfo; ii++)
	{
		if (MAX_GenIdxTemp < pInfo[ii].gn_idx)
		{
			MAX_GenIdxTemp = pInfo[ii].gn_idx;
		}
	}
	m_WTPara.MAX_GenOld_idx = MAX_GenIdxTemp;
#endif // _NUSP
...
int CWamToolObj::GetLfoAcln(vector<WT_DEV_INFO>& VecDevInfo)
{
	...
	{
		sprintf(m_LogString, "读取LFO交流线路表失败");
		MessShow(m_LogString, LTERROR);
		return -1;
	}
	...
	if (nInfo <= 0)
	{
		sprintf(m_LogString, "LFO交流线路记录为空[%d]", nInfo);
		MessShow(m_LogString, LTERROR);
		return 0;
	}
	else
	{
		sprintf(m_LogString, "LFO交流线路表[%d]", nInfo);
		MessShow(m_LogString, LTDEBUG);
	}
...
#ifdef _NUSP
	INT_REC MAX_AclnIdxTemp = pInfo[0].acln_idx;
	for (ii = 0; ii < nInfo; ii++)
	{
		if (MAX_AclnIdxTemp < pInfo[ii].acln_idx)
		{
			MAX_AclnIdxTemp = pInfo[ii].acln_idx;
		}
	}
	m_WTPara.MAX_AclnOld_idx = MAX_AclnIdxTemp;
#endif // _NUSP
...
int CWamToolObj::GetLfoTr(vector<WT_DEV_INFO>& VecDevInfo)
{
	...
	if (nInfo <= 0)
	{
		sprintf(m_LogString, "LFO变压器记录为空[%d]", nInfo);
		MessShow(m_LogString, LTERROR);
		return 0;
	}
	else
...
#ifdef _NUSP
	INT_REC MAX_TrIdxTemp = pInfo[0].tr_idx;
	for (ii = 0; ii < nInfo; ii++)
	{
		if (MAX_TrIdxTemp < pInfo[ii].tr_idx)
		{
			MAX_TrIdxTemp = pInfo[ii].tr_idx;
		}
	}
	m_WTPara.MAX_TrOld_idx = MAX_TrIdxTemp;
#endif // _NUSP
...
int CWamToolObj::GetVBase(vector<WT_DEV_INFO>& VecDevInfo, vector<VL_TYPE_TAB_CRE>& VecVLTypeTab)
{
	...
	for (ii = 0; ii < (int)VecDevInfo.size(); ii++)
	{
		...
		if (nFlag == 0)
		{
			sprintf(m_LogString, "["INT_REC_FMT"][%s]未找到对应电压等级[%d]", INT_REC_VAL(VecDevInfo[ii].dev_id), VecDevInfo[ii].dev_name, VecDevInfo[ii].maxvlty_id);
			MessShow(m_LogString, LTERROR);
		}
	}
	return 0;
...
int CWamToolObj::SetYcFlag(vector<WT_DEV_INFO>& VecDevInfo, vector<FES_YC_TAB_CRE>& VecFesYcTab, const int tableno)
{
	int ii, jj, kk, nNum;
	INT_REC tmprecord_id;
	int tmpcolumn_id;
	if (tableno == LFO_FAC_NO_TAB)
	{
		// 填充WAMS前置遥测信息
...
//					}
	...
	else if (tableno == LFO_GN_NO_TAB)
	{
		...
		for (ii = 0; ii < (int)VecDevInfo.size(); ii++)
		{
			...
			for (jj = 0; jj < (int)VecFesYcTab.size(); jj++)
			{
				...
				nNum = PODB_IsFlagAll(VecDevInfo[ii].dot1.nYcFlag, VEC_VIP_NUM);
				if (nNum >= VEC_VIP_NUM)
				{
					VecDevInfo[ii].dot1.IsYcDefine = WT_MENU_YES;
				}
	...
	else if (tableno == LFO_ACLN_NO_TAB)
	{
		// 填充WAMS前置遥测信息
				...
				if (VecDevInfo[ii].dot2.dot_id == tmprecord_id)
				{
					...
					for (kk = 0; kk < VEC_VIP_NUM; kk++)
					{
						if (tmpcolumn_id == m_VecColLn[kk])
						{
							...
							VecDevInfo[ii].dot2.nYcFilNo[kk] = VecFesYcTab[jj].file_no;
							VecDevInfo[ii].dot2.nYcColNo[kk] = VecFesYcTab[jj].col_no;
						}
					}
				}
				if (PODB_IsFlagAll(VecDevInfo[ii].dot1.nYcFlag, VEC_VIP_NUM) == VEC_VIP_NUM)
				{
					VecDevInfo[ii].dot1.IsYcDefine = WT_MENU_YES;
				}
				if (PODB_IsFlagAll(VecDevInfo[ii].dot2.nYcFlag, VEC_VIP_NUM) == VEC_VIP_NUM)
				{
	...
	else if (tableno == LFO_TR_NO_TAB)
	{
		...
		for (ii = 0; ii < (int)VecDevInfo.size(); ii++)
		{
			...
			for (jj = 0; jj < (int)VecFesYcTab.size(); jj++)
			{
				...
				if (VecDevInfo[ii].dot3.dot_id == tmprecord_id)
				{
					...
					for (kk = 0; kk < VEC_VIP_NUM; kk++)
					{
						if (tmpcolumn_id == m_VecColTr[kk])
						{
							...
							VecDevInfo[ii].dot3.nYcFilNo[kk] = VecFesYcTab[jj].file_no;
							VecDevInfo[ii].dot3.nYcColNo[kk] = VecFesYcTab[jj].col_no;
						}
					}
				}
				if (PODB_IsFlagAll(VecDevInfo[ii].dot1.nYcFlag, VEC_VIP_NUM) == VEC_VIP_NUM)
				...
				if (PODB_IsFlagAll(VecDevInfo[ii].dot3.nYcFlag, VEC_VIP_NUM) == VEC_VIP_NUM)
				{
...
int CWamToolObj::SetAddFlag(vector<WT_DEV_INFO>& VecDevInfo, const int tableno)
{
	...
	{
		// 发电机不判电压等级
		if (VecDevInfo[ii].dev_type == TOOL_DEV_TYPE_GEN)
		{
...
int CWamToolObj::WriteDevNew(vector<WT_DEV_INFO>& VecDevInfo, const int tableno)
{
	...
	if (tableno == LFO_FAC_NO_TAB)
	{
		...
		vector<LFO_FC_TAB_CRE> VecLfoFacW;
		LFO_FC_TAB_CRE tmpLfoFacW;
		for (ii = 0; ii < (int)VecDevInfo.size(); ii++)
		{
			...
			memset((void*)&tmpLfoFacW, 0, sizeof(LFO_FC_TAB_CRE));
#ifdef _NUSP
			tmpLfoFacW.fac_idx = PODB_ChangeDevId(LFO_FAC_NO_TAB, VecDevInfo[ii].dev_id);
#endif // _NUSP
			tmpLfoFacW.fac_id = VecDevInfo[ii].dev_id;
		...
		{
			sprintf(m_LogString, "新增厂站%d:"INT_REC_FMT"", ii + 1, INT_REC_VAL(VecLfoFacW[ii].fac_id));
			MessShow(m_LogString, LTDEBUG);
		}
		...
		for (ii = 0; ii < (int)VecLfoFacW.size(); ii++)
		{
			sprintf(SqlBuffer, ""LFO_FC_TAB_CRE_FMT"",
				tablename.c_str(),
				LFO_FC_TAB_CRE_FLD,
				LFO_FC_TAB_CRE_VAL(VecLfoFacW[ii]));
			VecSql.push_back(SqlBuffer);
		}
#ifdef _NUSP
		LFO_FAC_WAMS* add_fac_info = NULL;
		add_fac_info = new LFO_FAC_WAMS[add_num];
		memset((void*)add_fac_info, 0, add_num * sizeof(LFO_FAC_WAMS));
		//IncrRtdbId(tmpIdx, add_num - 1);
		for (ii = 0; ii < add_num; ii++)
		{
			add_fac_info[ii].fac_idx = PODB_ChangeDevId(LFO_FAC_NO_TAB, VecLfoFacW[add_num - 1 - ii].fac_id);
			add_fac_info[ii].fac_id = VecLfoFacW[add_num - 1 - ii].fac_id;
		...
		int ret_code = PODB_TableWrite((const char*)&add_fac_info[0].fac_idx, add_num * sizeof(LFO_FAC_WAMS), tableno);
		if (ret_code < 0)
		{
			sprintf(m_LogString, "写LFO厂站表[app：%d，table：%d]失败，记录数[%d]\n", g_appNo, tableno, add_num);
			MessShow(m_LogString);
		}
...
//			tmpIdx = PODB_GetDevIdByKeyID(tableno, m_WTPara.nGenOld + 1);  
		...
		vector<LFO_GN_TAB_CRE> VecLfoGenW;
		LFO_GN_TAB_CRE tmpLfoGenW;
		for (ii = 0; ii < (int)VecDevInfo.size(); ii++)
		{
			if (VecDevInfo[ii].is_add == WT_MENU_NO)
			{
			...
			memset((void*)&tmpLfoGenW, 0, sizeof(LFO_GN_TAB_CRE));
#ifdef _NUSP
			tmpLfoGenW.gn_idx = PODB_ChangeDevId(LFO_GN_NO_TAB, VecDevInfo[ii].dev_id);
#endif // _NUSP			
			tmpLfoGenW.gn_id = VecDevInfo[ii].dev_id;
			tmpLfoGenW.fac_id = VecDevInfo[ii].fac_id;
		...
		for (ii = 0; ii < (int)VecLfoGenW.size(); ii++)
		{
			sprintf(SqlBuffer, ""LFO_GN_TAB_CRE_FMT"",
				tablename.c_str(),
				LFO_GN_TAB_CRE_FLD,
				LFO_GN_TAB_CRE_VAL(VecLfoGenW[ii]));
			VecSql.push_back(SqlBuffer);
...
#ifdef _NUSP
		LFO_GN_WAMS* add_gen_info = NULL;
		add_gen_info = new LFO_GN_WAMS[add_num];
		memset((void*)add_gen_info, 0, add_num * sizeof(LFO_GN_WAMS));
		for (ii = 0; ii < add_num; ii++)
		{
			add_gen_info[ii].gn_idx = PODB_ChangeDevId(LFO_GN_NO_TAB, VecLfoGenW[add_num - 1 - ii].gn_id);
			add_gen_info[ii].gn_id = VecLfoGenW[add_num - 1 - ii].gn_id;
			add_gen_info[ii].fac_id = VecLfoGenW[add_num - 1 - ii].fac_id;
		...
		int ret_code = PODB_TableWrite((const char*)&add_gen_info[0].gn_idx, add_num * sizeof(LFO_GN_WAMS), tableno);
		if (ret_code < 0)
		{
			sprintf(m_LogString, "写LFO发电机表[app：%d，table：%d]失败，记录数[%d]\n", g_appNo, tableno, add_num);
			MessShow(m_LogString);
		}
	...
	else if (tableno == LFO_ACLN_NO_TAB)
	{
		...
		vector<LFO_LN_TAB_CRE> VecLfoLnW;
		LFO_LN_TAB_CRE tmpLfoLnW;
		for (ii = 0; ii < (int)VecDevInfo.size(); ii++)
		{
			if (VecDevInfo[ii].is_add == WT_MENU_NO)
			{
			...
			memset((void*)&tmpLfoLnW, 0, sizeof(LFO_LN_TAB_CRE));
#ifdef _NUSP
			tmpLfoLnW.acln_idx = PODB_ChangeDevId(LFO_ACLN_NO_TAB, VecDevInfo[ii].dev_id);
#endif // _NUSP
		...
		{
			sprintf(SqlBuffer, ""LFO_LN_TAB_CRE_FMT"",
				tablename.c_str(),
				LFO_LN_TAB_CRE_FLD,
				LFO_LN_TAB_CRE_VAL(VecLfoLnW[ii]));
			VecSql.push_back(SqlBuffer);
			// 			sprintf(m_LogString,"%s",SqlBuffer);
			// 			MessShow(m_LogString,LTINFO);
		}
#ifdef _NUSP
		LFO_ACLN_WAMS* add_acln_info = NULL;
		add_acln_info = new LFO_ACLN_WAMS[add_num];
		memset((void*)add_acln_info, 0, add_num * sizeof(LFO_ACLN_WAMS));
		for (ii = 0; ii < add_num; ii++)
		{
			add_acln_info[ii].acln_idx = PODB_ChangeDevId(LFO_ACLN_NO_TAB, VecLfoLnW[add_num - 1 - ii].acln_id);
		...
		int ret_code = PODB_TableWrite((const char*)&add_acln_info[0].acln_idx, add_num * sizeof(LFO_ACLN_WAMS), tableno);
		if (ret_code < 0)
		{
			sprintf(m_LogString, "写LFO交流线路表[app：%d，table：%d]失败，记录数[%d]\n", g_appNo, tableno, add_num);
			MessShow(m_LogString);
		}
		else
		{
			sprintf(m_LogString, "写LFO交流线路表[app：%d，table：%d]成功，记录数[%d]\n", g_appNo, tableno, add_num);
			MessShow(m_LogString);
		}
	...
	else if (tableno == LFO_TR_NO_TAB)
	{
		...
		vector<LFO_TR_TAB_CRE> VecLfoTrW;
		LFO_TR_TAB_CRE tmpLfoTrW;
		for (ii = 0; ii < (int)VecDevInfo.size(); ii++)
		{
			if (VecDevInfo[ii].is_add == WT_MENU_NO)
			{
			...
			memset((void*)&tmpLfoTrW, 0, sizeof(LFO_TR_TAB_CRE));
#ifdef _NUSP
			tmpLfoTrW.tr_idx = PODB_ChangeDevId(LFO_TR_NO_TAB, VecDevInfo[ii].dev_id);
#endif // _NUSP
			tmpLfoTrW.tr_id = VecDevInfo[ii].dev_id;
			tmpLfoTrW.fac_id = VecDevInfo[ii].fac_id;
			...
			tmpLfoTrW.mon_wind_slct = HIGH_WINDING;
		...
		{
			sprintf(SqlBuffer, ""LFO_TR_TAB_CRE_FMT"",
				tablename.c_str(),
				LFO_TR_TAB_CRE_FLD,
				LFO_TR_TAB_CRE_VAL(VecLfoTrW[ii]));
			VecSql.push_back(SqlBuffer);
		}
#ifdef _NUSP
		LFO_TR_WAMS* add_tr_info = NULL;
		add_tr_info = new LFO_TR_WAMS[add_num];
		memset((void*)add_tr_info, 0, add_num * sizeof(LFO_TR_WAMS));
		for (ii = 0; ii < add_num; ii++)
		{
			add_tr_info[ii].tr_idx = PODB_ChangeDevId(LFO_TR_NO_TAB, VecLfoTrW[add_num - 1 - ii].tr_id);
			add_tr_info[ii].tr_id = VecLfoTrW[add_num - 1 - ii].tr_id;
			add_tr_info[ii].fac_id = VecLfoTrW[add_num - 1 - ii].fac_id;
		...
		int ret_code = PODB_TableWrite((const char*)&add_tr_info[0].tr_idx, add_num * sizeof(LFO_TR_WAMS), tableno);
		if (ret_code < 0)
		{
			sprintf(m_LogString, "写LFO变压器表[app：%d，table：%d]失败，记录数[%d]\n", g_appNo, tableno, add_num);
			MessShow(m_LogString);
		}
		else
		{
			sprintf(m_LogString, "写LFO变压器表[app：%d，table：%d]成功，记录数[%d]\n", g_appNo, tableno, add_num);
			MessShow(m_LogString);
		}
...
int CWamToolObj::WriteDevFile(vector<WT_DEV_INFO>& VecDevInfo, const int tableno)
{
	...
	if (tableno == LFO_FAC_NO_TAB)
	{
		sprintf(m_LogString, "厂站信息-表号[%d]-厂站[%d]-PMU厂站[%d]-LFO厂站[%d]-新增[%d]", LFO_FAC_NO_TAB, m_WTPara.nFacAll, m_WTPara.nFacPmu, m_WTPara.nFacOld, m_WTPara.nFacNew);
		MessShow(m_LogString);
	...
	{
		sprintf(m_LogString, "发电机信息-表号[%d]-发电机[%d]-PMU发电机[%d]-LFO发电机[%d]-新增[%d]", LFO_GN_NO_TAB, m_WTPara.nGenAll, m_WTPara.nGenPmu, m_WTPara.nGenOld, m_WTPara.nGenNew);
		MessShow(m_LogString);
	...
	{
		sprintf(m_LogString, "交流线路信息-表号[%d]-交流线路[%d]-PMU交流线路[%d]-LFO交流线路[%d]-新增[%d]", LFO_ACLN_NO_TAB, m_WTPara.nAclnAll, m_WTPara.nAclnPmu, m_WTPara.nAclnOld, m_WTPara.nAclnNew);
		MessShow(m_LogString);
	...
	{
		sprintf(m_LogString, "变压器信息-表号[%d]-变压器[%d]-PMU变压器[%d]-LFO变压器[%d]-新增[%d]", LFO_TR_NO_TAB, m_WTPara.nTrAll, m_WTPara.nTrPmu, m_WTPara.nTrOld, m_WTPara.nTrNew);
		MessShow(m_LogString);
...
int CWamToolObj::WriteToolInf(const string ToolInf)
{
	...
	{
#ifdef _WAMAP_
		int idx = PODB_GetTabRecNum(LFO_PARA_NO_TAB);
		INT_REC recordid = PODB_GetDevIdByKeyID(LFO_PARA_NO_TAB, 1);
		char ana_run_ctx[64];
		char LogString[1024];
		sprintf(ana_run_ctx, "%s", ToolInf.c_str());
		sprintf(LogString, "WriteToolInf更新参数表[%s],表号[%d],记录ID["INT_REC_FMT"]记录数[%d]", ana_run_ctx, LFO_PARA_NO_TAB, INT_REC_VAL(recordid), idx);
		MessShow(LogString);

		PODB_TableModifyByKey((const char*)&recordid, "ana_run_ctx", (char*)&ana_run_ctx, sizeof(ana_run_ctx), LFO_PARA_NO_TAB);
#endif
	}

...
//广东项目增加
	...
	float lfo_gn_swn;	/* 功率振荡开始振幅限值 */
	float lfo_gn_ewn;	/* 功率振荡结束振幅限值 */
#ifdef _CHG_AMPL
...
//广东项目增加
	...
	float lfo_ln_swn;	/* 低频振荡开始振幅限值 */
	float lfo_ln_ewn;	/* 低频振荡结束振幅限值 */
#ifdef _CHG_AMPL
	int is_chg_ampl;	/* 是否计算动态振幅限值 */
	float chg_ampl_per;	/* 动态振幅限值百分比 */
#endif // _CHG_AMPL
...
#else
	int lfo_is_disp2;	/* lfo是否显示2 */
	float lfo_tr_swn;	/* 低频振荡开始振幅限值 */
	float lfo_tr_ewn;	/* 低频振荡结束振幅限值 */
#ifdef _CHG_AMPL
	int is_chg_ampl;	/* 是否计算动态振幅限值 */
	float chg_ampl_per;	/* 动态振幅限值百分比 */
#endif // _CHG_AMPL
...
{
    INT_REC inf_idx;	/* 走廊索引 */
    INT_REC inf_no;	/* 走廊编号 */
    char inf_name[64];	/* 走廊名称 */
    unsigned char is_monitor;	/* 是否监视 */
	unsigned char lfo_is_disp2;	/* 是否显示 */
} LFO_INT_TAB_CRE;
...
{
	INT_REC recordid;	/* 顺序号 */
	INT_REC inf_no;	/* 走廊编号 */
	char inf_name[64];	/* 走廊名称 */
	int eletype;	/* 主变/线路 */
	char line_name[64];	/* 支路名称 */
	INT_REC line_no;	/* 支路编号 */
	int onoff;	/* 支路投停 */
	int mpoint;	/* 测点 */

...
int CPubOdbApi::PODB_InitColID(vector<int>& VecCol, const int tableno)
{
	...
	{
		// SSF SSP SS_STAT 
		VecCol.push_back(PODB_GetFieldNoByName(GN_DEVICE_NO_TAB, SSF, 304));
		VecCol.push_back(PODB_GetFieldNoByName(GN_DEVICE_NO_TAB, SSP, 294));
		VecCol.push_back(PODB_GetFieldNoByName(GN_DEVICE_NO_TAB, SSO, 296));
		VecCol.push_back(PODB_GetFieldNoByName(GN_DEVICE_NO_TAB, P_VAL, 70));
		if ((int)VecCol.size() < VEC_SSO_NUM)
		{
	...
	{
		// SSF SSP SS_STAT 
		VecCol.push_back(PODB_GetFieldNoByName(ACLN_DOT_NO_TAB, SSF, 90));
		VecCol.push_back(PODB_GetFieldNoByName(ACLN_DOT_NO_TAB, SSP, 95));
		VecCol.push_back(PODB_GetFieldNoByName(ACLN_DOT_NO_TAB, SSO, 30));
		VecCol.push_back(PODB_GetFieldNoByName(ACLN_DOT_NO_TAB, P_VAL, 30));
		if ((int)VecCol.size() < VEC_SSO_NUM)
		{
			sprintf(m_LogString, "列号个数错误[%d]", (int)VecCol.size());
			MessShow(m_LogString, LTERROR);
	...
	{
		// SSF SSP SS_STAT 
		VecCol.push_back(PODB_GetFieldNoByName(TRWD_DEVICE_NO_TAB, SSF, 181));
		VecCol.push_back(PODB_GetFieldNoByName(TRWD_DEVICE_NO_TAB, SSP, 187));
		VecCol.push_back(PODB_GetFieldNoByName(TRWD_DEVICE_NO_TAB, SSO, 183));
		VecCol.push_back(PODB_GetFieldNoByName(TRWD_DEVICE_NO_TAB, P_VAL, 50));
		if ((int)VecCol.size() < VEC_SSO_NUM)
		{
			sprintf(m_LogString, "列号个数错误[%d]", (int)VecCol.size());
			MessShow(m_LogString, LTERROR);
...
#ifdef _WAMAP_
	CBuffer buf_base;
	if (PODB_TableGet(WAM_FES_YC_DEFINE_NO_TAB, FES_YC_TAB_CRE_FLD, buf_base, AP_WAMS_FES) < 0)
	{
		return -1;
	}
...
int CPubOdbApi::PODB_GetFesYxInfo(vector<FES_YX_TAB_CRE>& VecFesYxTab)
{
	if (!VecFesYxTab.empty())
	{
		VecFesYxTab.clear();
	}
#ifdef _WAMAP_
	CBuffer buf_base;
	if (PODB_TableGet(WAM_FES_YX_DEFINE_NO_TAB, FES_YX_TAB_CRE_FLD, buf_base, AP_WAMS_FES) < 0)
	{
		return -1;
	}

	int nInfo = buf_base.GetLength() / sizeof(FES_YX_TAB_CRE);
	if (nInfo <= 0)
	{
		sprintf(m_LogString, "WAMS前置遥信定义表记录为空[%d]", nInfo);
		MessShow(m_LogString, LTERROR);
		return 0;
	}
	else
	{
		sprintf(m_LogString, "WAMS前置遥信定义表[%d]", nInfo);
		MessShow(m_LogString);
	}

	FES_YX_TAB_CRE* pInfo = NULL;
	pInfo = new FES_YX_TAB_CRE[nInfo];
	memcpy(pInfo, buf_base.GetBufPtr(), nInfo * sizeof(FES_YX_TAB_CRE));

	int i = 0;
	for (i = 0; i < nInfo; i++)
	{
		VecFesYxTab.push_back(pInfo[i]);
	}
...
int CPubOdbApi::PODB_GetFesCOMMFACInfo(vector<FES_COMM_TAB_CRE>& VecFesCOMMTab)
{
	if (!VecFesCOMMTab.empty())
	{
		VecFesCOMMTab.clear();
	}
#ifdef _WAMAP_
#ifndef _KEDONG
	CBuffer buf_base;
	if (PODB_TableGet(WAM_COMM_FAC_INFO_NO, FES_COMM_TAB_CRE_FLD, buf_base, AP_WAMS_FES) < 0)
	{
		return -1;
	}

	int nInfo = buf_base.GetLength() / sizeof(FES_COMM_TAB_CRE);
	if (nInfo <= 0)
	{
		sprintf(m_LogString, "WAMS前置通讯厂站表记录为空[%d]", nInfo);
		MessShow(m_LogString, LTERROR);
		return 0;
	}
	else
	{
		sprintf(m_LogString, "WAMS前置通讯厂站表[%d]", nInfo);
		MessShow(m_LogString, LTDEBUG);
	}

	FES_COMM_TAB_CRE* pInfo = NULL;
	pInfo = new FES_COMM_TAB_CRE[nInfo];
	memcpy(pInfo, buf_base.GetBufPtr(), nInfo * sizeof(FES_COMM_TAB_CRE));

	int i = 0;
	for (i = 0; i < nInfo; i++)
	{
		sprintf(m_LogString, "通讯ID[%ld],厂站ID[%ld],厂站名[%s]", pInfo[i].comm_fac_id, pInfo[i].st_id, pInfo[i].fac_name);
		MessShow(m_LogString);
		VecFesCOMMTab.push_back(pInfo[i]);
	}
...
int CPubOdbApi::PODB_YxYc(vector<FES_YC_TAB_CRE>& VecFesYcTab, vector<FES_YX_TAB_CRE>& VecFesYxTab, vector<FES_YX_YC>& VecFesYcYx)
{
	int ii;
	FES_YX_YC tmp_yx_yc;
	INT_REC tmprecord_id;
	for (ii = 0; ii < VecFesYcTab.size(); ii++)
	{
		if (!VALID_INT_REC(VecFesYcTab[ii].fac_id) || !VALID_INT_REC(VecFesYcTab[ii].yc_id) || VecFesYcTab[ii].dot_no < 0 || VecFesYcTab[ii].file_no < 0 || VecFesYcTab[ii].col_no < 0)
		{
			continue;
		}
		tmp_yx_yc.st_id = VecFesYcTab[ii].fac_id;
		tmp_yx_yc.yc_id = VecFesYcTab[ii].yc_id;
		tmp_yx_yc.dot_no = VecFesYcTab[ii].dot_no;
		tmp_yx_yc.file_no = VecFesYcTab[ii].file_no;
		tmp_yx_yc.col_no = VecFesYcTab[ii].col_no;
		VecFesYcYx.push_back(tmp_yx_yc);
	}
	for (ii = 0; ii < VecFesYxTab.size(); ii++)
	{
		if (!VALID_INT_REC(VecFesYxTab[ii].st_id) || !VALID_INT_REC(VecFesYxTab[ii].yx_id) || VecFesYxTab[ii].dot_no < 0 || VecFesYxTab[ii].file_no < 0 || VecFesYxTab[ii].col_no < 0)
		{
			continue;
		}
		tmp_yx_yc.st_id = VecFesYxTab[ii].st_id;
		tmp_yx_yc.yc_id = VecFesYxTab[ii].yx_id;
		tmp_yx_yc.dot_no = VecFesYxTab[ii].dot_no;
		tmp_yx_yc.file_no = VecFesYxTab[ii].file_no;
		tmp_yx_yc.col_no = VecFesYxTab[ii].col_no;
		VecFesYcYx.push_back(tmp_yx_yc);
	}
	sprintf(m_LogString, "前置遥信定义表记录为[%d],前置遥测定义表记录为[%d],合并后记录数为[%d]", VecFesYxTab.size(), VecFesYcTab.size(), VecFesYcYx.size());
	MessShow(m_LogString);
...
int CPubOdbApi::PODB_SendMessageToHost(const char* message, const int msg_type, const int length, const char* host_name, const char* process_name, const char* instance_name)
{
	...
	int ret_code = m_MsgBus->SendMessageToHost(message, msg_type, length, host_name, process_name);
	if (ret_code < 0)
	{
...
int CPubOdbApi::PODB_TriggerSample(vector<TRIGGER_SAMPLE_DEFINE>& vectrisample)
{
	...
	ret_code = PODB_SendMessageToHost(msg_buf, DB_TRIGGER_SAMPLE_TYPE, msg_size, tmphostname.c_str(), "TRIGGER_SAMPLE_SERVER");
	if (ret_code < 0)
	{
...
int CPubOdbApi::PODB_SendWarnMessage(vector<WARN_MESSAGE_STRUCT>& tmpVecWarnMsg, const int appno, const int contextno)
{
	...
	ret_code = PODB_SendMessageToHost(msg_buf, APP_TO_WARN_SERVICE_TYPE, buf_size, hostname.c_str(), "WARN_SERVICE");
	SAFE_DELETE(msg_buf);
	if (ret_code < 0)
	{
